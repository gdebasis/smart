 ...
 *1 retrieve.rank_tr.rank_did_nsim
 *2 rank_did_nsim (new, results, inst)
 *3   TOP_RESULT *new;
 *3   RESULT *results;
 *3   int inst;
...
int
rank_did_nsim (new, results, inst)
TOP_RESULT *new;
RESULT *results;
int inst;
{
    ...
    if (results->min_top_sim == -1.0e8 && results->top_results[0].did == 0)
        min_sim_index = 0;
