#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libretrieve/sim_cty_inv_lm_lda.c,v 11.2 1993/02/03 16:52:55 smart Exp $";
#endif
...
static char *prefix;
static char *inv_file, *lda_inv_file;
static long inv_mode, lda_inv_mode;
static float ctype_weight;
static float lambda /*weight for tf */, lda_lambda /* weight for lda */;
...
static SPEC_PARAM_PREFIX pspec_args[] = {
    ...
    {&prefix, "lda_inv_file",     getspec_dbfile,    (char *) &lda_inv_file},
    {&prefix, "lda_inv_file.rmode", getspec_filemode,(char *) &lda_inv_mode},
...
static SPEC_PARAM spec_args[] = {
    ...
    {"retrieve.lm.lda_lambda", getspec_float, (char *) &lda_lambda},  // this parameter is the initial lambda of LM
    TRACE_PARAM ("retrieve.sim_ctype_inv_lm_lda.trace")
};
...
int
init_sim_ctype_inv_lm_lda (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_inv_lm_lda");
    ...
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_inv_lm_lda");
        return (UNDEF);
    }
    if (! VALID_FILE (lda_inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant LDA inverted file", "sim_ctype_inv_lm_lda");
        return (UNDEF);
    }

    PRINT_TRACE (4, print_string, inv_file);
    PRINT_TRACE (4, print_string, lda_inv_file);
    ...
    if (UNDEF == (ip->inv_fd = open_inv(inv_file, inv_mode)))
        return (UNDEF);
    if (UNDEF == (ip->lda_inv_fd = open_inv(lda_inv_file, lda_inv_mode)))
        return (UNDEF);

    ip->ctype_weight = ctype_weight; 
    ip->valid_info++;

    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_inv_lm_lda");
    return (new_inst);
}

int
sim_ctype_inv_lm_lda (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    INV inv, lda_inv;
    register LISTWT *listwt_ptr, *lda_listwt_ptr, *end_listwt_ptr;
    TOP_RESULT new_top;
    VEC new_qvec ;
	long  conwt_offset ;
	char msg[1024];
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_inv_lm_lda");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_inv_lm_lda");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    /* Perform inverted file retrieval, sequentially going through query.
       ...
       because this invoked through sim_vec_inv_lm_lda)
	*/
	num_conwt = new_qvec.num_conwt;
    ...
    for ( query_con = new_qvec.con_wtp, conwt_offset = 0; 
        ...
        lda_inv.id_num = query_con->con;
        if (1 != seek_inv (ip->inv_fd, &inv) ||
            1 != read_inv (ip->inv_fd, &inv)) {
            continue;
        }
        if (1 != seek_inv (ip->lda_inv_fd, &lda_inv) ||
            1 != read_inv (ip->lda_inv_fd, &lda_inv)) {
            continue;
        }

        end_listwt_ptr = &inv.listwt[inv.num_listwt];
        for (listwt_ptr = inv.listwt, lda_listwt_ptr = lda_inv.listwt;
             listwt_ptr < end_listwt_ptr;
             listwt_ptr++, lda_listwt_ptr++) {
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_inv_lm_lda");
                    return (UNDEF);
                }
            ...
            results->full_results[listwt_ptr->list] +=
                query_wt * log(1 +
						lambda/(1-lambda-lda_lambda)*listwt_ptr->wt +
						lda_lambda/(1-lambda-lda_lambda)*lda_listwt_ptr->wt) ;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_inv_lm_lda");
    return (1);
}


int
close_sim_ctype_inv_lm_lda (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_inv_lm_lda");
    if (! VALID_INST (inst)) {
    ...
    if (ip->valid_info == 0) {
        ...
        if (UNDEF == close_inv (ip->inv_fd))
            return (UNDEF);
        if (UNDEF == close_inv (ip->lda_inv_fd))
            return (UNDEF);
    }

    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_inv_lm_lda");
    return (0);
}
