#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libretrieve/sim_cty_inv_lm_len.c,v 11.2 1993/02/03 16:52:55 smart Exp $";
#endif
...
static SPEC_PARAM spec_args[] = {
    ...
    TRACE_PARAM ("retrieve.sim_ctype_inv_lm_len.trace")
};
...
int
init_sim_ctype_inv_lm_len (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_inv_lm_len");
    ...
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_inv_lm_len");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_inv_lm_len");
    return (new_inst);
}
...
int
sim_ctype_inv_lm_len (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_inv_lm_len");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_inv_lm_len");
        return (UNDEF);
    }
    ip  = &info[inst];
	...
	*/
	...
	qryLength = getDocLength(&new_qvec);
    ...
    for ( query_con = new_qvec.con_wtp, conwt_offset = 0; 
        ...
        for (listwt_ptr = inv.listwt;
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_inv_lm_len");
                    return (UNDEF);
                }
            ...
            if ( results->full_results[listwt_ptr->list] == 0 ) {
                ...
                results->full_results[listwt_ptr->list] = docPriorSgn*log(1 + MIN(docLength, qryLength)/(float)MAX(docLength, qryLength));
            }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_inv_lm_len");
    return (1);
}


int
close_sim_ctype_inv_lm_len (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_inv_lm_len");
    if (! VALID_INST (inst)) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_inv_lm_len");
    return (0);
}
