...
int
init_sim_vec_inv_lm (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_vec_inv_lm");
    ...
    if (max_did > max_did_in_coll) {
		...
		if (NULL == (full_results = Malloc (max_did_in_coll + 1, float))) {
	    	set_error (errno, "", "sim_vec_inv_lm");
		    return (UNDEF);
		}
    }

    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_vec_inv_lm");
    return (new_inst);
}

int
sim_vec_inv_lm (in_retrieval, results, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_vec_inv_lm");
    PRINT_TRACE (6, print_vector, (VEC *) query_vec->vector);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_vec_inv_lm");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_vec_inv_lm");
    return (1);
}


int
close_sim_vec_inv_lm (inst)
int inst;
{
    long i;
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_sim_vec_inv_lm");
    ...
    ip->valid_info = 0;
    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_vec_inv_lm");
    return (0);
}
