#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libretrieve/sim_cty_inv_lm_nsim.c,v 11.2 1993/02/03 16:52:55 smart Exp $";
#endif
...
int
init_sim_ctype_inv_lm_nsim (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_inv_lm_nsim");
    ...
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_inv_lm_nsim");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_inv_lm_nsim");
    return (new_inst);
}
...
int sim_ctype_inv_lm_nsim (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_inv_lm_nsim");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_inv_lm_nsim");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    /* Perform inverted file retrieval, sequentially going through query.
       ...
       because this invoked through sim_vec_inv_lm_nsim)
	*/
    ...
    for ( query_con = new_qvec.con_wtp, conwt_offset = 0; 
        ...
        for (listwt_ptr = inv.listwt;
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_inv_lm_nsim");
                    return (UNDEF);
                }
            ...
            results->full_results[listwt_ptr->list] += listwt_ptr->wt; // -ve numbers are being accumulated
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_inv_lm_nsim");
    return (1);
}


int
close_sim_ctype_inv_lm_nsim (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_inv_lm_nsim");
    if (! VALID_INST (inst)) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_inv_lm_nsim");
    return (0);
}
