...
int
init_sim_ctype_inv_avgpos (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_inv_avgpos");
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_inv_avgpos");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_inv_avgpos");
    return (new_inst);
}

// We need to get the vector length from the average positional vector.
// Note that we store only the average position of each term in
// the vector. The best approximation to length can thus be obtained
// by the maximum position in this vector.
static float getAvgPosVecLength(VEC* vec) {
	float maxPos = 0;
	CON_WT* con_wtp;

	for (con_wtp = vec->con_wtp; con_wtp < &vec->con_wtp[vec->num_conwt]; con_wtp++) {
		if (con_wtp->wt > maxPos)
			maxPos = con_wtp->wt;
	}
	return maxPos;
}

static float getDocumentLength(STATIC_INFO* ip, int id)
{
	VEC  vec;
    vec.id_num.id = id;
	...
	return getAvgPosVecLength(&vec);
}


int
sim_ctype_inv_avgpos (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    INV inv;
    register LISTWT *listwt_ptr, *end_listwt_ptr, *prev_listwt_ptr;
    TOP_RESULT new_top;
	int qcon_offset;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_inv_avgpos");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_inv_avgpos");
        return (UNDEF);
    }
    ip  = &info[inst];
	...
	qryLength = getAvgPosVecLength(qvec);

    for ( query_con = qvec->con_wtp; query_con < last_query_con; query_con++) {
        ...
        for (listwt_ptr = inv.listwt, prev_listwt_ptr = NULL;
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_inv_avgpos");
                    return (UNDEF);
                }
            ...
            if (results->full_results[listwt_ptr->list] == 0) {
                docLength = getDocumentLength(ip, listwt_ptr->list);
                results->full_results[listwt_ptr->list] = log(1 + MIN(docLength, qryLength)/(float)MAX(docLength, qryLength)) ;
            }
			pos_abs_diff = fabs(query_con->wt - listwt_ptr->wt);
			posdiff_comp = log (1 + 1/(pos_abs_diff + delta));
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_inv_avgpos");
    return (1);
}


int
close_sim_ctype_inv_avgpos (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_inv_avgpos");
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ctype_inv_avgpos");
        return (UNDEF);
    }
    ip  = &info[inst];
    ip->valid_info--;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_inv_avgpos");
    return (0);
}
