...
int
init_sim_ctype_invpos (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_invpos");
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_invpos");
        return (UNDEF);
    }
    ...
    if (! VALID_FILE (collstat_file)) {
		ip->collstats_info.freq = NULL ;
       	ip->collstats_info.num_freq = 0;
    }
    else {
		...
		if (1 != seek_dir_array (collstats_fd, &dir_array) ||
		    ...
		    if (NULL == (ip->collstats_info.freq = (long *)
		                 malloc ((unsigned) dir_array.num_list)))
		        return (UNDEF);
	    	(void) bcopy (dir_array.list,
	                  (char *) ip->collstats_info.freq,
	                  dir_array.num_list);
		    ip->collstats_info.num_freq = dir_array.num_list / sizeof (long);
		}
	...
	ip->totalDocFreq = getTotalDocFreq(ip);

    ip->ctype_weight = ctype_weight;
    ip->valid_info++;

    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_invpos");
    return (new_inst);
}

static float getTotalDocFreq(STATIC_INFO* ip) {
	long i;
	long totalDocFreq = 0 ;
	for (i = 0; i < ip->collstats_info.num_freq; i++) {
		totalDocFreq += ip->collstats_info.freq[i] ;
	}
...
int
sim_ctype_invpos (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_invpos");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_invpos");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    for ( query_con = qvec->con_wtp + qryLength;	// start of the positional information 
        ...
        for (listwt_ptr = inv.listwtpos, prev_listwt_ptr = NULL;
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_invpos");
                    return (UNDEF);
                }
			...
			if (prev_listwt_ptr) {
				if (listwt_ptr->list == prev_listwt_ptr->list) {
					...
					///1.posdiff_comp = log (1 + 1/(minpos_diff + delta));
					posdiff_comp = (qryLength - minpos_diff)* (ip->totalDocFreq/(float)ip->collstats_info.freq[query_con->con]+1);
					///2.posdiff_comp = (MIN(docLength, qryLength) - (minpos_diff + delta)) * log(ip->totalDocFreq/(float)ip->collstats_info.freq[query_con->con]+1);
            		results->full_results[prev_listwt_ptr->list] += posdiff_comp;
					minpos_diff = pos_abs_diff;
				}
			}
			else {
            ...
            if (results->full_results[listwt_ptr->list] == 0) {
                ...
                ///1.results->full_results[listwt_ptr->list] = log(1 + MIN(docLength, qryLength)/(float)MAX(docLength, qryLength)) ;
                results->full_results[listwt_ptr->list] = MIN(docLength, qryLength)/(float)MAX(docLength, qryLength);
            }
		...
		posdiff_comp = (qryLength - minpos_diff)* (ip->totalDocFreq/(float)ip->collstats_info.freq[query_con->con]+1);
		///2.posdiff_comp = (MIN(docLength, qryLength) - (minpos_diff + delta)) * log(ip->totalDocFreq/(float)ip->collstats_info.freq[query_con->con]+1);
   		results->full_results[end_listwt_ptr->list] += posdiff_comp;
    }

    PRINT_TRACE (20, print_full_results, results);
    PRINT_TRACE (4, print_top_results, results);
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_invpos");
    return (1);
}


int
close_sim_ctype_invpos (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_invpos");
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ctype_invpos");
        return (UNDEF);
    }
    ip  = &info[inst];
    ip->valid_info--;
