#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libretrieve/sim_cty_inv_lm_idf.c,v 11.2 1993/02/03 16:52:55 smart Exp $";
#endif
...
static SPEC_PARAM spec_args[] = {
    ...
    TRACE_PARAM ("retrieve.sim_ctype_inv_lm_idf.trace")
};
...
int
init_sim_ctype_inv_lm_idf (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    DIR_ARRAY dir_array;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_inv_lm_idf");
    ...
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_inv_lm_idf");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_inv_lm_idf");
    return (new_inst);
}
...
int
sim_ctype_inv_lm_idf (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_inv_lm_idf");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_inv_lm_idf");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    /* Perform inverted file retrieval, sequentially going through query.
       ...
       because this invoked through sim_vec_inv_lm_idf)
	*/
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_inv_lm_idf");
                    return (UNDEF);
                }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_inv_lm_idf");
    return (1);
}

int
close_sim_ctype_inv_lm_idf (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_inv_lm_idf");
    if (! VALID_INST (inst)) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_inv_lm_idf");
    return (0);
}
