 ...
 *1 local.feedback.best_terms.ret
 *2 best_terms_ret (num_to_add, fdbk_info, inst)
 *3   long *num_to_add;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_best_terms_ret (spec, param_prefix)
 *5   "feedback.best_terms.trace"
 *5   "ctype.*.inv_file"
 *5   "ctype.*.inv_file.rmode"
 *5   "ctype.*.sim_ctype_weight"
 *4 close_best_terms_ret (inst)
 ...
 *8 best_terms_ret is most often called by feedback.form_query.form_best()
***********************************************************************/
...
int
init_best_terms_ret (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_best_terms_ret");
    ...
    if (NULL == (rh = get_rel_header (textloc_file)) || 
        ...
        set_error (SM_ILLPA_ERR, "Cannot open textloc file", "best_terms_ret");
        return (UNDEF);
    }
    ...
    if (NULL == (full_results = (float *) 
        ...
        set_error (errno, "1", "best_terms_ret");
        return (UNDEF);
    }
    ...
    if (NULL == (inv_fd = (int *) 
        ...
        set_error (errno, "2", "best_terms_ret");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_best_terms_ret");
    return (new_inst);
}

int
best_terms_ret (fdbk_info, unused, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering best_terms_ret");
    PRINT_TRACE (6, print_fdbk_info, fdbk_info);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "best_terms_ret");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    if (qrels.qid !=  fdbk_info->orig_query->id_num.id ||
        ...
        for (i = 0; i < fdbk_info->num_occ; i++) {
            ...
            PRINT_TRACE (2, print_string, "Trace: leaving best_terms_ret");
        }
    }
    ...
    for (i = 0; i < fdbk_info->num_occ; i++) {
        if (fdbk_info->occ[i].query) {
            if (orig_ratio < 1.0 && fdbk_info->occ[i].weight > 0.0) {
                if (UNDEF == try_improve(&fdbk_info->occ[i],
                                         (1.0 - orig_ratio) *
    ...
    PRINT_TRACE (8, print_string, "After selection of terms\n");
...
int
close_best_terms_ret (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_best_terms_ret");
    if (! VALID_INST (inst)) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_best_terms_ret");
    return (0);
}
