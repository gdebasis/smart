 ...
 *2 wtfdbk_sel_q_all_clstridf (unused1, fdbk_info, inst)
 *3   char *unused1;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_wtfdbk_sel_q_all_clstridf (spec, unused)
 *5   "feedback.alpha"
 *5   "feedback.beta"
 *5   "feedback.gamma"
 *5   "feedback.weight.trace"
 *4 close_wtfdbk_sel_q_all_clstridf (inst)
...
int
init_wtfdbk_sel_q_all_clstridf (spec, unused)
SPEC *spec;
char *unused;
{
    REL_HEADER *rh;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_wtfdbk_sel_q_all_clstridf");

    if (NULL != (rh = get_rel_header (textloc_file)) && rh->num_entries > 0)
	num_docs = rh->num_entries;
    else {
	set_error (SM_INCON_ERR, textloc_file, "init_wtfdbk_sel_q_all_clstridf");
	return(UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_wtfdbk_sel_q_all_clstridf");
    return (1);
}

int
wtfdbk_sel_q_all_clstridf (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering wtfdbk_sel_q_all_clstridf");
...
printf("Selected %ld terms from cluster %ld\n", 
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving wtfdbk_sel_q_all_clstridf");

    return (1);
}

int
close_wtfdbk_sel_q_all_clstridf (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_wtfdbk_sel_q_all_clstridf");
