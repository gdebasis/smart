...
static long extra_idf;

static SPEC_PARAM spec_args[] = {
    {"feedback.fdbk_stats_file",  getspec_dbfile, (char *) &fdbk_stats_file},
    {"feedback.fdbk_stats_file.rwcmode",
     			getspec_filemode,(char *)&fdbk_stats_rwcmode},
    {"feedback.num_ctypes",	getspec_long, (char *) &num_ctypes},
    {"feedback.extra_idf",	getspec_bool, (char *) &extra_idf},
...
int
init_local_occ_inv_stats (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_local_occ_inv_stats");
	...
	if (UNDEF == (idf_inst[i] = init_con_cw_idf(spec, param_prefix)))
	    return UNDEF;
    }

    PRINT_TRACE (2, print_string, "Trace: leaving init_local_occ_inv_stats");
    return (1);
}

int
local_occ_inv_stats (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering local_occ_inv_stats");
    ...
    for (i = 0; i < fdbk_info->num_occ; i++) {
        ...
        if (fdbk_info->occ[i].ctype >= num_ctypes) {
            set_error (SM_INCON_ERR, "Ctype too large", "local_occ_inv_stats");
            return (UNDEF);
        }
	if (UNDEF != inv_fd[fdbk_info->occ[i].ctype]) {
		...
		printf("local_occ_info: concept %ld not found in inv_file\n",
		       fdbk_info->occ[i].con);
		continue;
	    }
	...
	if (extra_idf) {
	    if (UNDEF == con_cw_idf(&fdbk_info->occ[i].con, &idf,
				    idf_inst[fdbk_info->occ[i].ctype]))
		return UNDEF;
	    fdbk_info->occ[i].rel_weight *= idf;
	    fdbk_info->occ[i].nrel_weight *= idf;
	}
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_occ_inv_stats");
...
int
close_local_occ_inv_stats (inst)
int inst;
{
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_local_occ_inv_stats");
    ...
    for (i = 0; i < num_ctypes; i++) {
	    ...
	    UNDEF == close_inv(inv_fd[i]))
            return (UNDEF);
        inv_fd[i] = UNDEF;
	if (UNDEF == close_con_cw_idf(idf_inst[i]))
	    return UNDEF;
    }

    PRINT_TRACE (2, print_string, "Trace: leaving close_local_occ_inv_stats");
    return (1);
}
