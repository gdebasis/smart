...
int
init_local_occ_inv (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_local_occ_inv");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_local_occ_inv");
    return (1);
}

int
local_occ_inv (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering local_occ_inv");

    for (i = 0; i < fdbk_info->num_occ; i++) {
        inv.id_num = fdbk_info->occ[i].con;
        if (fdbk_info->occ[i].ctype >= num_ctypes) {
            set_error (SM_INCON_ERR, "Ctype too large", "local_occ_inv");
            return (UNDEF);
        }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_occ_inv");
...
int
close_local_occ_inv (inst)
int inst;
{
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_local_occ_inv");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_local_occ_inv");
    return (1);
}
