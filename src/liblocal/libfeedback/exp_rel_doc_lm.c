#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libfeedback/local_exp_reldoc_lm.c,v 11.0 1992/07/21 18:20:39 chrisb Exp $";
#endif
...
/* 
 * Implement the LM term selection value (as proposed by Ponte
 * in his PhD. thesis) which is defined as:
 * L(t) = summation over all d in R ( LM_score(t|d) / df(t) )
 * R being the set of relevant documents.
*/
...
int
init_local_exp_reldoc_lm (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_local_exp_reldoc_lm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_local_exp_reldoc_lm");
    return (1);
}

int
local_exp_reldoc_lm (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering local_exp_reldoc_lm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_exp_reldoc_lm");
    return (1);
}

int
close_local_exp_reldoc_lm (inst)
int inst;
{
	long i;
    PRINT_TRACE (2, print_string, "Trace: entering close_local_exp_reldoc_lm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_local_exp_reldoc_lm");
    return (0);
}
...
int orig_query_flag;
{
    ...
    for (ctype = 0; ctype < dvec->num_ctype; ctype++) {
        ...
        while (conwt < end_conwt) {
			// conwt->wt contains the LM score for this term
			// (without the log and the lambda factor). Divide it by df(t).
			lm_score = log( (lambda/(1 - lambda) * conwt->wt) / (double)collstats_info[ctype].freq[conwt->con] * num_docs);
            if (ctype < old_occ_ctype ||
                ...
                new_occ->rel_weight = old_occ->rel_weight + lm_score;
