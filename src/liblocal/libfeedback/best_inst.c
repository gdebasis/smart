 ...
 *2 best_ret_inst (num_to_add, fdbk_info, inst)
 *3   long *num_to_add;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_best_ret_inst (spec, param_prefix)
 *5   "feedback.best_terms.trace"
 *5   "ctype.*.inv_file"
 *5   "ctype.*.inv_file.rmode"
 *5   "ctype.*.sim_ctype_weight"
 *4 close_best_ret_inst (inst)
 ...
 *8 best_ret_inst is most often called by feedback.form_query.form_best()
***********************************************************************/
...
typedef struct {
    ...
    long exp_num_success[6];
    long exp_success[6][300];
    float init_sim;
    float add_sim;
    float round_sim[6];
} RET_TRACE_INFO;
...
int
init_best_ret_inst (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_best_ret_inst");
    ...
    if (NULL == (rh = get_rel_header (textloc_file)) || 
        ...
        set_error (SM_ILLPA_ERR, "Cannot open textloc file", "best_ret_inst");
        return (UNDEF);
    }
    ...
    if (NULL == (full_results = (float *) 
        ...
        set_error (errno, "1", "best_ret_inst");
        return (UNDEF);
    }
    ...
    if (NULL == (inv_fd = (int *) 
        ...
        set_error (errno, "2", "best_ret_inst");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_best_ret_inst");
    return (new_inst);
}

int
best_ret_inst (fdbk_info, unused, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering best_ret_inst");
    PRINT_TRACE (6, print_fdbk_info, fdbk_info);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "best_ret_inst");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    if (qrels.qid !=  fdbk_info->orig_query->id_num.id ||
        ...
        for (i = 0; i < fdbk_info->num_occ; i++) {
            ...
            PRINT_TRACE (2, print_string, "Trace: leaving best_ret_inst");
        }
    }
...
#ifdef TRACE
    ret_trace_info.init_sim += current_eval;
#endif  /* TRACE */
...
#ifdef TRACE
    ret_trace_info.add_sim += current_eval;
#endif  /* TRACE */
    ...
    for (j = 0; j < num_best_ratio; j++) {
        ...
        PRINT_TRACE (8, print_fdbk_info, fdbk_info);
#ifdef TRACE
        if (j < 6) ret_trace_info.round_sim[j] += current_eval;
#endif  /* TRACE */
    }
...
static int
try_improve (occinfo, try_incr_weight)
OCC *occinfo;
...
int
close_best_ret_inst (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_best_ret_inst");
    if (! VALID_INST (inst)) {
    ...
    if (sm_trace >= 5) {
        ...
        fprintf (global_trace_fd, "Avg_num_first_incr %8.2f Avg_num_exp_add %8.2f\n  Success_pass_0 %8.2f Success_pass_1 %8.2f Success_pass_2 %8.2f\n",
                 ...
                 ret_trace_info.num_queries);
        fprintf (global_trace_fd, "  Success_pass_3 %8.2f Success_pass_4 %8.2f Success_pass_5 %8.2f\n",
                 (double) ret_trace_info.exp_num_success[3] /
                 ret_trace_info.num_queries,
                 (double) ret_trace_info.exp_num_success[4] /
                 ret_trace_info.num_queries,
                 (double) ret_trace_info.exp_num_success[5] /
                 ret_trace_info.num_queries);
        fprintf (global_trace_fd, "  Sims: Init %6.4f Add %6.4f Pass %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f\n",
                 (double) ret_trace_info.init_sim /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.add_sim /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.round_sim[0] /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.round_sim[1] /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.round_sim[2] /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.round_sim[3] /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.round_sim[4] /
                         ret_trace_info.num_queries,
                 (double) ret_trace_info.round_sim[5] /
                         ret_trace_info.num_queries);
        for (i = 0; i < 300; i++) {
        ...
        for (i = 0; i < 300; i++) {
            if (ret_trace_info.exp_success[0][i] > 0) 
                fprintf (global_trace_fd, 
                         " e_incr  expterm=%4ld, pass: #0 %4.2f, #1 %4.2f, #2 %4.2f #3 %4.2f #4 %4.2f #5 %4.2f\n",
                         i,
                         ...
                         ret_trace_info.num_queries,
                         (double) ret_trace_info.exp_success[3][i] / 
                         ret_trace_info.num_queries,
                         (double) ret_trace_info.exp_success[4][i] / 
                         ret_trace_info.num_queries,
                         (double) ret_trace_info.exp_success[5][i] / 
                         ret_trace_info.num_queries);
        }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_best_ret_inst");
    return (0);
}
