 ...
 *2 best_ret_inst_fast (num_to_add, fdbk_info, inst)
 *3   long *num_to_add;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_best_ret_inst_fast (spec, param_prefix)
 *5   "feedback.best_terms.trace"
 *5   "ctype.*.inv_file"
 *5   "ctype.*.inv_file.rmode"
 *5   "ctype.*.sim_ctype_weight"
 *4 close_best_ret_inst_fast (inst)
 ...
 *8 best_ret_inst_fast is most often called by feedback.form_query.form_best()
***********************************************************************/
...
int
init_best_ret_inst_fast (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_best_ret_inst_fast");
    ...
    if (NULL == (rh = get_rel_header (textloc_file)) || 
        ...
        set_error (SM_ILLPA_ERR, "Cannot open textloc file", "best_ret_inst_fast");
        return (UNDEF);
    }
	...
	/* +1 before I am in a hurry and don't want to figure usage in rank_did */
        ...
        set_error (errno, "1", "best_ret_inst_fast");
        return (UNDEF);
    }
    ...
    if (NULL == (inv_fd = (int *) 
        ...
        set_error (errno, "2", "best_ret_inst_fast");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_best_ret_inst_fast");
    return (new_inst);
}

int
best_ret_inst_fast (fdbk_info, unused, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering best_ret_inst_fast");
    PRINT_TRACE (6, print_fdbk_info, fdbk_info);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "best_ret_inst_fast");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    if (qrels.qid !=  fdbk_info->orig_query->id_num.id ||
        ...
        for (i = 0; i < fdbk_info->num_occ; i++) {
            ...
            PRINT_TRACE (2, print_string, "Trace: leaving best_ret_inst_fast");
        }
    }
...
int
close_best_ret_inst_fast (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_best_ret_inst_fast");
    if (! VALID_INST (inst)) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_best_ret_inst_fast");
    return (0);
}
