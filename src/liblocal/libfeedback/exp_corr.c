 ...
 *2 exp_corr (unused1, fdbk_info, inst)
 *3   char *unused1;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_exp_corr (spec, unused)
 *4 close_exp_corr (inst)
...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_exp_corr");
	...
	set_error (SM_ILLPA_ERR, textloc_file, "exp_corr");
	return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_exp_corr");
    return (1);
}

int
exp_corr (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering exp_corr");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving exp_corr");
...
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_exp_corr");
...
#define CORREL(p12,p1,p2) (p1==1.0 || p2==1.0 || p1==0.0 || p2==0.0 ? 0.0: \
            ...
            if (global_trace && sm_trace >= 8) {
                fprintf(global_trace_fd, "%ld\t%ld\t%ld:%s:%ld\t%ld\t%ld\t%ld\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%c%c\n",
