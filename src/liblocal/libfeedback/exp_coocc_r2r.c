#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libfeedback/exp_coocc_r2r.c,v 11.2 1993/02/03 16:50:03 smart Exp $";
#endif
 ...
 *0 Procedure to generate query term cooccurrences based on R + top 2R
 *2 exp_coocc_r2r (unused1, fdbk_info, inst)
 *3   char *unused1;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_exp_coocc_r2r (spec, unused)
 *4 close_exp_coocc_r2r (inst)
...
static SPEC_PARAM spec_args[] = {
    ...
    {"feedback.coocc.textloc_file", getspec_dbfile, (char *) &textloc_file},
...
int
init_exp_coocc_r2r (spec, unused)
SPEC *spec;
char *unused;
{
    CONTEXT old_context;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_exp_coocc_r2r");
	...
	set_error (SM_ILLPA_ERR, textloc_file, "exp_coocc_r2r");
	return (UNDEF);
    }
    num_docs = rh->num_entries + 1;
    max_num_occ = 0;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_exp_coocc_r2r");
    return (1);
}

int
exp_coocc_r2r (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering exp_coocc_r2r");

    for (i = 0; i < fdbk_info->num_occ && fdbk_info->occ[i].ctype == 0; i++)
        if (fdbk_info->occ[i].weight != 0.0)
	    num_terms++;

    if (max_num_occ < num_terms*(num_terms-1)/2) {
	if (max_num_occ)
	    Free(occ);
	max_num_occ = num_terms*(num_terms-1)/2;
	if (NULL == (occ = Malloc(max_num_occ, OCC)))
	    return(UNDEF);
    }
	...
	UNDEF == merge_stats(&coocc_info, fdbk_info))
	return (UNDEF);

    PRINT_TRACE (4, print_fdbk_info, fdbk_info);
    PRINT_TRACE (2, print_string, "Trace: leaving exp_coocc_r2r");

    return (1);
}

int
close_exp_coocc_r2r (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_exp_coocc_r2r");
...
FEEDBACK_INFO *coocc_info;
{
    ...
    long num_words, num_ret, rel_count, nrel_count, min_rel;
    ...
    /* assign and save fdbk_info->tr->num_tr */
	...
	inv1.id_num = fdbk_info->occ[i].con;
	if (1 != seek_inv(inv_fd, &inv1) ||
	    1 != read_inv(inv_fd, &inv1)) {
            PRINT_TRACE (1, print_string, "exp_coocc_r2r: concept not found in inv_file:");
            PRINT_TRACE (1, print_long, &inv1.id_num);
	    continue;
	}

	for (j = i+1; j < num_words; j++) { 
	    if (fdbk_info->occ[j].weight == 0.0)
		continue;
	    inv2.id_num = fdbk_info->occ[j].con;
	    if (1 != seek_inv(inv_fd, &inv2) ||
		1 != read_inv(inv_fd, &inv2)) {
                PRINT_TRACE (1, print_string, "exp_coocc_r2r: concept not found in inv_file:");
                PRINT_TRACE (1, print_long, &inv2.id_num);
		continue;
	    }
	    ...
	    /* get idf weights */
	    co_idf = (float) log((double) num_docs/(double) co_df);
