#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libfeedback/local_exp_const_lm.c,v 11.2 1993/02/03 16:49:57 smart Exp $";
#endif
...
/* Expand query by adding at least num_expand terms occurring in rel docs.
   ...
   The difference of local.feedback.local_exp_const_lm with feedback.exp_const_lm is that
   it uses the rel_weight to order the terms. The function local.feedback.exp_rel_doc_lm
   stores the LM weights.
*/
...
int init_local_exp_reldoc_lm(), local_exp_reldoc_lm(), close_local_exp_reldoc_lm();

static SPEC *save_spec;


int
init_local_exp_const_lm (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_local_exp_const_lm");

    if (UNDEF == (local_exp_rel_doc_inst = init_local_exp_reldoc_lm (spec, (char *) NULL)))
        return (UNDEF);

    save_spec = spec;

    PRINT_TRACE (2, print_string, "Trace: leaving init_local_exp_const_lm");
    return (1);
}

int
local_exp_const_lm (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering local_exp_const_lm");

    /* Get all terms occurring in rel docs */
    if (UNDEF == local_exp_reldoc_lm (unused, fdbk_info, local_exp_rel_doc_inst))
        return (UNDEF);
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_exp_const_lm");
    return (1);
}

int
close_local_exp_const_lm (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_local_exp_const_lm");

    if (UNDEF == close_local_exp_reldoc_lm (local_exp_rel_doc_inst))
        return (UNDEF);

    PRINT_TRACE (2, print_string, "Trace: leaving close_local_exp_const_lm");
    return (0);
}
