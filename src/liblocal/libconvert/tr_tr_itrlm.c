...
int
init_tr_tr_itrlm (spec, unused)
SPEC *spec;
char *unused;
{
	DIR_ARRAY dir_array;

    PRINT_TRACE (2, print_string, "Trace: entering/leaving init_tr_tr_itrlm");
	ip.spec = spec;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_tr_tr_itrlm");
    return 1;
}

int
tr_tr_itrlm (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering tr_tr_itrlm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving tr_tr_itrlm");
    return (1);
}


int
close_tr_tr_itrlm (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_tr_tr_itrlm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_tr_tr_itrlm");
    return (0);
}
...
static int rerank_tr_trlm(VEC* qvec)
{
	...
	if (UNDEF == init_ilda_est_inst(&ip.ldamodel, doc_fd, &tr, qvec, multifaceted)) {
