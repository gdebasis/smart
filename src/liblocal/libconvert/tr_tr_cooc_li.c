 ...
 *2 rerank_coocc (in_tr_file, out_tr_file, inst)
 *3 char *in_tr_file;
 *3 char *out_tr_file;
 *3 int inst;
 *4 init_rerank_coocc (spec, unused)
 *5   "rerank.in_tr_file"
 *5   "rerank.in_tr_file.rmode"
 *5   "rerank.out_tr_file"
 *5   "rerank.trace"
 *4 close_rerank_coocc (inst)
...
static int get_corr(), num_matches();
...
int inst;
{
	...
	if (UNDEF == qid_vec(&(qvec.id_num.id), &qvec, qid_vec_inst))
	    return(UNDEF);
	prune_query(&qvec);
	while (this_query < corr + num_corr &&
	       this_query->qid < qvec.id_num.id)
	    this_query++;
	for (i=0; i < top; i++) {
	    newsim = 0;
            /*BUG-FIX. get_condoc might return 0 if can't find doc */
	    if (UNDEF == (status = get_condoc(&(tr_vec.tr[i].did), &condoc, condoc_inst)))
		return(UNDEF);
            if (status == 0) continue;
	    if (UNDEF == num_matches(&qvec, &condoc, this_query, &newsim))
		return(UNDEF);
...
VEC *qvec;
{
	...
	sscanf(inbuf, "%ld%d%ld%d%ld%ld%ld%ld\n",
	       &cptr->qid, &ctype1, &cptr->con1, &ctype2, &cptr->con2, 
	       &cptr->common, &cptr->count1, &cptr->count2);
        if (ctype1 == 1 || ctype2 == 1) continue; // handling ctype 0 only!!
	num_corr++;
	cptr++;
...
#if 0
#define LOCAL_DF(ret, df) (10/log(1 + df/(ret + 0.0001)))
#else
#define LOCAL_DF(ret, df) 1.0
#endif

static int
num_matches(query, doc, this_query, sim)
VEC *query;
SM_CONDOC *doc;
CORRELATION *this_query;
float *sim;
