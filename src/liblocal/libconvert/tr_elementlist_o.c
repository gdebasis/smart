...
typedef struct {
    ...
    unsigned char *path;
    long section_num; //, start_offset, end_offset;
    float sim;
...
static int qid_vec_inst, pp_inst, index_pp_inst, vv_inst, print_vec_inst;
...
int
init_tr_elementlist (spec, unused)
SPEC *spec;
char *unused;
{
    CONTEXT old_context;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_tr_elementlist");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_tr_elementlist");
...
int
tr_elementlist (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering tr_elementlist");
    ...
    while (1 == read_tr_vec (in_fd, &tr_vec)) {
        ...
        num_elements = 0;
		for (i=0; i < num_topdocs/*tr_vec.num_tr*/; i++) {
            ...
            for (j = 0; j < sm_disp->num_sections; j++) {
                ...
                if (UNDEF == index_pp->proc(&element, &evec, index_pp_inst) ||
                    UNDEF == sim_vec_vec(&vpair, &sim, vv_inst))
                    return UNDEF;
                ...
                element_sims[num_elements].num_chars = element.textloc_doc.end_text - element.textloc_doc.begin_text;
                num_elements++;
            }

            /* Eliminate overlaps: if two elements are overlapping, keep
             * only the higher ranked one.
             */
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving tr_elementlist");
    return (1);
}

int
close_tr_elementlist (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_tr_elementlist");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_tr_elementlist");
    return (0);
}
...
TR_TUP *tr;
{
    ...
    fprintf(stderr, 
            "\n============================================================\n"
            "Document %ld: before elimination of duplicates"
            "\n============================================================\n",
            tr->did);
    for (i = 0; i < n; i++) {
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = i+1; j < n; j++) {
            ...
            if ((this->begin_section <= that->begin_section &&
                 this->end_section >= that->end_section) ||
                (this->begin_section >= that->begin_section &&
                 this->end_section <= that->end_section))
                /* flag an overlapping section by making sim negative */
                elements[j].sim = -1.0;
        }
    }
    ...
    fprintf(stderr, 
            "\n============================================================\n"
            "Document %ld: after elimination of duplicates"
            "\n============================================================\n",
            tr->did);
    for (i = 0; i < n; i++) {
        fprintf(stderr, "%ld: %s\n%.6f\n",
