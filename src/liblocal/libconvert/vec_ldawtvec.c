 ...
 *
 * Take a SMART vector file (typically the one used for retrieval, e.g. LM)
 * and append the LDA weights to it. For the time being doing a SUPER-LAZY
 * implementation, i.e. just adding the weighted contribution of the LDA
 * probabilities to the LM probabilities instead of keeping them
 * in separate CON_WTS.
 * i.e. wt := 0.7*lm_wt + 0.3*lda_wt
 * For the experiments, we use a hard-coded value of lambda=0.7 (as reported
 * in Liu and Croft).
 *
 * This conversion is to be performed before generating the inverted files.
