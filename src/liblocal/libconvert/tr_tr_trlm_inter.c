 ...
 *
 * Interactive TRLM feedback: Instead of drawing in contributions from
 * all topics in TRLM feedback, rerank documents based on the contribution
 * from a user specified topic.
...
static SPEC_PARAM spec_args[] = {
    ...
    {"tr_tr_trlm.src_ldamodel_name", getspec_string, (char *) &ip.srcmodel_name},
    {"tr_tr_trlm.fdbk_topic", getspec_int, (char *) &fdbk_topic},
...
int
init_tr_tr_trlm_inter (spec, unused)
SPEC *spec;
char *unused;
{
	DIR_ARRAY dir_array;

    PRINT_TRACE (2, print_string, "Trace: entering/leaving init_tr_tr_trlm_inter");
	ip.spec = spec;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_tr_tr_trlm_inter");
    return 1;
}

int
tr_tr_trlm_inter (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering tr_tr_trlm_inter");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving tr_tr_trlm_inter");
    return (1);
}


int
close_tr_tr_trlm_inter (inst)
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_tr_tr_trlm_inter");
    return (0);
}
...
static int rerank_tr_trlm(VEC* qvec)
{
	...
	if (UNDEF == init_lda_est_ref_inst(&ip.ldamodel, ip.srcmodel_name, qvec)) {
	...
	for (tr_ind = 0; tr_ind < tr.num_tr; tr_ind++) {
		...
		for (conwtp = dvec.con_wtp; conwtp < &dvec.con_wtp[dvec.ctype_len[0]]; conwtp++) {
			...
			p_w_D = mixing_lambda*compute_single_topic_P_w_D(&ip.ldamodel, conwtp, rel_count, fdbk_topic) +
