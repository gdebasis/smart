 ...
 *1 convert.obj.local_text_vec_obj
 *2 local_text_vec_obj (text_file, vec_file, inst)
 *3 char *text_file;
 *3 char *vec_file;
 *3 int inst;

 *4 init_local_text_vec_obj (spec, unused)
 *5   "local_text_vec_obj.vec_file"
 *5   "local_text_vec_obj.vec_file.rwmode"
 *5   "local_text_vec_obj.trace"
 *4 close_local_text_vec_obj (inst)
 ...
 *7 giving the weight of concept con with ctype ctype occurring in did. 
  ...
  * Debasis: The difference with the libconvert/text_vec_o is that
  * the input might comprise of multiple cons from two possible sources
  * when two smprint vec outputs are concatenated and sorted. This
  * conversion routine merges two such smprint dumps into a single
  * vector.
...
int
init_local_text_vec_obj (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_local_text_vec_obj");
    PRINT_TRACE (2, print_string, "Trace: leaving init_local_text_vec_obj");

    return (0);
}

int
local_text_vec_obj (text_file, vec_file, inst)
char *text_file;
char *vec_file;
int inst;
{
    ...
    long i;
    PRINT_TRACE (2, print_string, "Trace: entering local_text_vec_obj");
    ...
    while (NULL != fgets (line_buf, PATH_LEN, in_fd)) {
        ...
        if (textline.num_tokens < 4) {
            set_error (SM_ILLPA_ERR, "Illegal input line", "local_text_vec_obj");
            return (UNDEF);
        }
        ...
        con = atol (textline.tokens[2]);
		if (old_con != con) {
			if (old_con != UNDEF) {
        		vec.con_wtp[vec.num_conwt].con = old_con;
        		vec.con_wtp[vec.num_conwt].wt  = conwt;
        		vec.num_conwt++;
				conwt = 0;
			}
			else
				conwt += atof (textline.tokens[3]);
			old_con = con;
		}
		conwt += atof (textline.tokens[3]);
	...
	vec.con_wtp[vec.num_conwt].con = con;
   	vec.con_wtp[vec.num_conwt].wt  = conwt;
    /* Output last vector */
    if (vec.id_num.id != 0) {
        if (UNDEF == seek_vector (vec_fd, &vec) ||
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_text_vec_obj");
    return (1);
}

int
close_local_text_vec_obj (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_local_text_vec_obj");
    PRINT_TRACE (2, print_string, "Trace: leaving close_local_text_vec_obj");
