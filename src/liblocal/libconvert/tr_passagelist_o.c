...
void reweightElement(VEC* dvec, VEC* evec, float lambda, float mu) {
	CON_WT *conwt, *dvec_conwt;

	for (conwt = evec->con_wtp; conwt < &evec->con_wtp[evec->num_conwt]; conwt++) {
		dvec_conwt = bsearch(conwt, dvec->con_wtp, dvec->num_conwt, sizeof(CON_WT), compare_conwt);
		assert(dvec_conwt);
...
TR_TUP *tr;
{
    ...
    xmlNodePtr xptr;
	ELEMENT_SIM* elt_sim;
	int thisContainedInThat, thatContainedInThis, thisIntersectsThat;
    ...
    for (i = 0; i < n; i++) {
		if (elements[i].sim == -1) continue;
        this = sm_disp->sections + elements[i].section_num;

        for (j = i+1; j < n; j++) {
			if (elements[j].sim == -1) continue;
            that = sm_disp->sections + elements[j].section_num;

			thisIntersectsThat = this->begin_section <= that->begin_section && this->end_section >= that->end_section;
			...
			if (!thisIntersectsThat && !thisContainedInThat && !thatContainedInThis)
				continue;
            /* Mark an overlapping section by making sim negative */
			if (elements[i].sim == elements[j].sim) {
				if (thisContainedInThat)
					elt_sim = &elements[j];
				else if (thatContainedInThis)
					elt_sim = &elements[i];
			}
			else {
				// Keep the one with greater similarity
				elt_sim = elements[i].sim > elements[j].sim? elements + j : elements + i;
			}
            elt_sim->sim = -1.0;
        }
    }
