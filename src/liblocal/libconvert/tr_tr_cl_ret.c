 ...
 *7 Similar to tr_tr_rerank.cluster-selall.c.
 *7 Uses actual relevance information (hence retro). Used to obtain
 *7 an estimate of the best results achievable using clustering.
***********************************************************************/
...
static float rel_density, factor;
...
static SPEC_PARAM spec_args[] = {
    ...
    {"rerank.rel_density",	getspec_float,	(char *)&rel_density},
...
int
init_rerank_cluster_retro (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_rerank_cluster_retro");
    ...
    if (top_wanted <= 0) {
	set_error(SM_ILLPA_ERR, "top_wanted", "init_rerank_cluster_retro");
	return(UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_rerank_cluster_retro");
...
int
rerank_cluster_retro (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
    ...
    long num_results, num_rel, ctype_num_terms, i, j, k;
    EID docid;
    EID_LIST elist;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering rerank_cluster_retro");
	...
	for (i = 0; i < top_wanted; i++)
	    if (cluster[i].num > 1) {
		for (j = 0, num_rel = 0; j < cluster[i].num; j++)
			...
			if (tr_vec.tr[k].did == cluster[i].eid[j].id) {
			    ...
			    num_rel += tr_vec.tr[k].rel;
			    break;
			}
		cluster[i].num_rel = num_rel;
		/* If this cluster has a sufficiently high rel_density,
		 * give positive similarities to it documents */
		if (100.0 * (float)num_rel / (float)cluster[i].num >= rel_density) {
	...
	if (cent_flag) {
		...
		if (cluster[i].num > 1) {
		    ...
		    /* Print info */
		    printf("\nCluster %ld (%ld/%ld):", j+1, 
			   cluster[i].num_rel, cluster[i].num);
		    ...
		    /* HACK: save the no. of documents and no. of rel. 
		     * documents for this cluster in the ext.num field. 
		     * Note: this is the point where this is different
		     * from cluster_selall. */
		    cent_vec.id_num.ext.num = cluster[i].num*100 + 
			cluster[i].num_rel;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving rerank_cluster_retro");
    return (1);
}

int
close_rerank_cluster_retro (inst)
int inst;
{
    int i;

    PRINT_TRACE (2, print_string, "Trace: entering close_rerank_cluster_retro");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_rerank_cluster_retro");
    return (0);
}
