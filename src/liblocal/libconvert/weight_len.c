#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libconvert/weight_len.c,v 11.2 1993/02/03 16:49:34 smart Exp $";
#endif
 ...
 *0 Weight an entire vector with wt_ctype procedures, then len normalize
 *1 convert.weight.weight_len
 *2 weight_len (invec, outvec, inst)
 *3   VEC *invec;
 *3   VEC *outvec;
 *3   int inst;
 *4 init_weight_len (spec, unused)
 *5   "ctype.*.weight_ctype"
 *5   "convert.weight_len.len_file"
 *5   "convert.weight_len.pivot"
 *5   "convert.weight_len.slope"
 *5   "convert.weight.trace"
 *4 close_weight_len (inst)
 ...
 *7     slope*len + (1-slope)*pivot
 *7 where len is the document's length read from a length file.
***********************************************************************/
...
#define NUM_DOC 164777

static char *len_file;
static float pivot;
static float slope;

static SPEC_PARAM spec_args[] = {
    {"convert.weight_len.len_file", getspec_string, (char *) &len_file},
    {"convert.weight_len.pivot",  getspec_float, (char *) &pivot},
    {"convert.weight_len.slope", getspec_float, (char *) &slope},
...
char *prefix;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_weight_len");
    ...
    ip->len_file = len_file;
    ip->pivot = pivot;
    ip->slope = slope;

    if (NULL == (ip->fp = fopen(ip->len_file, "r")))
	return UNDEF;
    if (NULL == (ip->doc_len = Malloc(NUM_DOC + 1, float)))
	return UNDEF;
    for (i = 0; i < NUM_DOC; i++)
	if (EOF == fscanf(ip->fp, "%ld %f", &did, &(ip->doc_len[i+1])) ||
	    did != i+1) {
	    set_error (SM_ILLPA_ERR, ip->len_file, "init_weight_len");
	    return UNDEF;
	}

    ip->valid_info = 1;

    PRINT_TRACE (2, print_string, "Trace: leaving init_weight_len");

    return (new_inst);
}

int
weight_len (invec, outvec, inst)
VEC *invec;
VEC *outvec;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering weight_len");
    PRINT_TRACE (6, print_vector, invec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "weight_len");
        return (UNDEF);
    }
    ip  = &info[inst];
	...
	if (invec->num_ctype > ip->doc_desc.num_ctypes) {
	    set_error (SM_INCON_ERR, "Doc has too many ctypes", "weight_len");
	    return (UNDEF);
	}
    ...
    if (invec->num_conwt == 0 || invec->ctype_len == 0) {
        ...
        PRINT_TRACE (2, print_string, "Trace: leaving weight_len");
        return (0);
    }
    ...
    len = ip->doc_len[invec->id_num.id];
    if (len == 0.0) {
	...
	norm_factor = ip->slope*len + (1.0 - ip->slope)*pivot;
        for (i = 0; i < outvec->num_conwt; i++)
	    outvec->con_wtp[i].wt /= norm_factor;
    }
...
int
close_weight_len (inst)
int inst;
{
    long i;
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_weight_len");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_weight_len");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_weight_len");
