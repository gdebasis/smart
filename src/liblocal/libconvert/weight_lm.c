/* The local version of the procedure convert.weight_lm. This procedure weights the vectors by the lamdas in contrast to the convert.weight_lm
 * function where the lambda weighting is done in the retrieval step. */
#include <math.h>
#include "common.h"
#include "param.h"
#include "functions.h"
...
static SPEC_PARAM spec_args[] = {
    {"convert.weight.lm.lambda", getspec_float, (char *) &lambda},
...
int local_init_lang_model_wt_lm(SPEC* spec, char* unused)
{
    DIR_ARRAY dir_array;
	long ctype ;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering local_init_lang_model_wt_lm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_init_lang_model_wt_lm");
    return (1);
}
...
*/
int local_lang_model_wt_lm(VEC* invec, VEC* outvec, int inst)
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering local_lang_model_wt_lm");
	...
	*/
	for ( 	ctype = 0, startInVec = invec->con_wtp, startOutVec = outvec->con_wtp;
	    ...
	    for (	invec_conwt_ptr = startInVec, outvec_conwt_ptr = startOutVec;
			...
			outvec_conwt_ptr->wt = log(1 + ratio * lambda/(1-lambda)) ;
    	}
	}

    PRINT_TRACE (2, print_string, "Trace: leaving local_lang_model_wt_lm");
    return 1 ;
}

int local_close_lang_model_wt_lm(int inst)
{
	long ctype ;

    PRINT_TRACE (2, print_string, "Trace: entering local_close_lang_model_wt_lm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving local_close_lang_model_wt_lm");
    return (1);
}
