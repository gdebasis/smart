#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_op_other.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Parse SGML-ish <OTHER_OPER>
 *1 local.infoneed.op.parse.other_oper
 *2 ifn_parse_other(ifn_query, ifn_node, inst)
 *3   IFN_QUERY *ifn_query;
 *3   IFN_TREE_NODE *ifn_node;
 *3   int inst;
 *4 init_ifn_parse_other(spec, char_op_table)
 *5   "index.parse.trace"
 *4 close_ifn_parse_other(inst)

 *7 Parse an SGML-ish OTHER_OPER, as in TIPSTER DN2.  First process a fixed
 *7 set of possible characteristics in the <OTHER_OPER ...> header, then
 ...
 *7 set.
 *7 Allow partial processing of other operators than OTHER_OPER, in case
...
static IFN_CHILDREN generic_kids[] = {
	...
	{ "OTHER_OPER_ARGS", IFN_ONE_OPTIONAL, UNDEF },
...
int
init_ifn_parse_other(spec, char_op_table)
SPEC *spec;
char *char_op_table;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ifn_parse_other");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ifn_parse_other");
...
int
ifn_parse_other(ifn_query, ifn_node, inst)
IFN_QUERY *ifn_query;
...
int inst;
{
    ...
    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "ifn_parse_other");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering ifn_parse_other");
    ...
    if (same_tag(stop_tag, "OTHER_OPER")) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ifn_parse_other");
    return(retval);
}

int
close_ifn_parse_other(inst)
int inst;
{
    STATIC_INFO *ip;

    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "close_ifn_parse_other");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering close_ifn_parse_other");

    (void) free((char *)ip->kids);
    (void) free((char *)ip->operand_kids);
    ip->valid_info = 0;

    PRINT_TRACE (2, print_string, "Trace: leaving close_ifn_parse_other");
    return(0);
}
