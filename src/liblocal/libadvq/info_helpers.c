#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_helpers.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Helper routines for info_op_*.c
 *1 none
 *2 infoneed_parse_kaks(ifn_query, pos, kaks, ifn_node)
 *3   IFN_QUERY *ifn_query;
 *3   int *pos;
 *3   char **kaks;
 *3   IFN_TREE_NODE *ifn_node;
 *2 infoneed_specialize_kids(kids, op_table)
 *3   IFN_CHILDREN *kids;
 *3   ADVQ_OP *op_table;
 *2 infoneed_specialize_operandkids(kids, op_table)
 *3   IFN_CHILDREN **kids;
 *3   ADVQ_OP *op_table;
 *2 infoneed_check_kids(ifn_node, kids, operand_kids)
 *3   IFN_TREE_NODE *ifn_node;
 *3   IFN_CHILDREN *kids;
 *3   IFN_CHILDREN *operand_kids;

 *7 infoneed_parse_kaks() sets ifn_node according to any SGML header
 *7 characteristics found in ifn_query at pos that also occur in kaks.
 *7 Returns UNDEF if other characteristics or bad syntax is found, else 0.
 *7
 *7 infoneed_specialize_kids() looks up string tags of possible children
 *7 and maps them to operator numbers from a particular instance of operators.
 *7 Returns UNDEF if some children can't be mapped, else 0.
 *7
 *7 infoneed_specialize_operatorkids() looks up string tags of those
 *7 productions classed as proper operands for operators that take generic
 *7 OPERANDs and maps them to operator numbers from a particular instance
 *7 of operators.
 *7 Returns UNDEF if some children can't be mapped, else 0.
 *7
 *7 infoneed_check_kids() checks the children of ifn_node, making sure
 *7 that all required children are there and all children there are allowed
 *7 Returns UNDEF if there's a problem, else 0.
***********************************************************************/
