#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_op_annattr.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Parse SGML-ish <ANN_ATTR>
 *1 local.infoneed.op.parse.ann_attr
 *2 ifn_parse_annattr (ifn_query, ifn_node, inst)
 *3   IFN_QUERY *ifn_query;
 *3   IFN_TREE_NODE *ifn_node;
 *3   int inst;
 *4 init_ifn_parse_annattr (spec, char_op_table)
 *5   "index.parse.trace"
 *4 close_ifn_parse_annattr (inst)

 *7 Parse an SGML-ish ANN_ATTR, as in TIPSTER DN2.  First process a fixed
 *7 set of possible characteristics in the <ANN_ATTR ...> header, then
 ...
 *7 set.
 *7 Allow partial processing of other operators than ANN_ATTR, in case
...
static IFN_CHILDREN generic_kids[] = {
	{ "ANN_TYPE", IFN_ONE_OPTIONAL, UNDEF },
	{ "ATTR_NAME", IFN_ONE_OPTIONAL, UNDEF },
	{ "ATTR_TYPE", IFN_ONE_OPTIONAL, UNDEF },
	{ "ATTR_VALUE", IFN_ONE_OPTIONAL, UNDEF },
	{ "", IFN_NONE, UNDEF }
};
...
int
init_ifn_parse_annattr(spec, char_op_table)
SPEC *spec;
char *char_op_table;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ifn_parse_annattr");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ifn_parse_annattr");
...
int
ifn_parse_annattr(ifn_query, ifn_node, inst)
IFN_QUERY *ifn_query;
...
int inst;
{
    ...
    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "ifn_parse_annattr");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering ifn_parse_annattr");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ifn_parse_annattr");
    return(retval);
}

int
close_ifn_parse_annattr(inst)
int inst;
{
    STATIC_INFO *ip;

    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "close_ifn_parse_annattr");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering close_ifn_parse_annattr");

    (void) free((char *)ip->kids);
    ip->valid_info = 0;

    PRINT_TRACE (2, print_string, "Trace: leaving close_ifn_parse_annattr");
    return(0);
}
