#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_op_dn2.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Parse SGML-ish <DN2>
 *1 local.infoneed.op.parse.dn2
 *2 ifn_parse_dn2(ifn_query, ifn_node, inst)
 *3   IFN_QUERY *ifn_query;
 *3   IFN_TREE_NODE *ifn_node;
 *3   int inst;
 *4 init_ifn_parse_dn2(spec, char_op_table)
 *5   "index.parse.trace"
 *4 close_ifn_parse_dn2(inst)

 *7 Parse an SGML-ish DN2, as in TIPSTER DN2.  First process a fixed
 *7 set of possible characteristics in the <DN2 ...> header, then
 ...
 *7 set.
 *7 Allow partial processing of other operators than DN2, in case
...
static char *kaks[] = { "OUTPUT_QUERY", "NO_OUTPUT_QUERY",
			"OUTPUT_DOCS", "NO_OUTPUT_DOCS",
			(char *)0 };
...
static IFN_CHILDREN generic_kids[] = {
	...
	{ "MERGE_INFO", IFN_ONE_OPTIONAL, UNDEF },
	{ "INFO_NEED", IFN_ONE_OR_MORE, UNDEF },
	{ "", IFN_NONE, UNDEF }
};
...
int
init_ifn_parse_dn2(spec, char_op_table)
SPEC *spec;
char *char_op_table;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ifn_parse_dn2");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ifn_parse_dn2");
...
int
ifn_parse_dn2(ifn_query, ifn_node, inst)
IFN_QUERY *ifn_query;
...
int inst;
{
    ...
    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "ifn_parse_dn2");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering ifn_parse_dn2");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ifn_parse_dn2");
    return(retval);
}

int
close_ifn_parse_dn2(inst)
int inst;
{
    STATIC_INFO *ip;

    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "close_ifn_parse_dn2");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering close_ifn_parse_dn2");

    (void) free((char *)ip->kids);
    ip->valid_info = 0;

    PRINT_TRACE (2, print_string, "Trace: leaving close_ifn_parse_dn2");
    return(0);
}
