#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_op_ineed.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Parse SGML-ish <INFO_NEED>
 *1 local.infoneed.op.parse.info_need
 *2 ifn_parse_ineed(ifn_query, ifn_node, inst)
 *3   IFN_QUERY *ifn_query;
 *3   IFN_TREE_NODE *ifn_node;
 *3   int inst;
 *4 init_ifn_parse_ineed(spec, char_op_table)
 *5   "index.parse.trace"
 *4 close_ifn_parse_ineed(inst)

 *7 Parse an SGML-ish INFO_NEED, as in TIPSTER DN2.  First process a fixed
 *7 set of possible characteristics in the <INFO_NEED ...> header, then
 ...
 *7 set.
 *7 Allow partial processing of other operators than INFO_NEED, in case
