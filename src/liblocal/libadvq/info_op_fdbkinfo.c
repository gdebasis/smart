#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_op_fdbkinfo.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Parse SGML-ish <FEEDBACK_INFO>
 *1 local.infoneed.op.parse.feedback_info
 *2 ifn_parse_fdbkinfo(ifn_query, ifn_node, inst)
 *3   IFN_QUERY *ifn_query;
 *3   IFN_TREE_NODE *ifn_node;
 *3   int inst;
 *4 init_ifn_parse_fdbkinfo(spec, char_op_table)
 *5   "index.parse.trace"
 *4 close_ifn_parse_fdbkinfo(inst)

 *7 Parse an SGML-ish FEEDBACK_INFO, as in TIPSTER DN2.  First process a fixed
 *7 set of possible characteristics in the <FEEDBACK_INFO ...> header, then
 ...
 *7 set.
 *7 Allow partial processing of other operators than FEEDBACK_INFO, in case
...
static IFN_CHILDREN generic_kids[] = {
	{ "DOCID_REL", IFN_ZERO_OR_MORE, UNDEF },
	{ "TEXT_REL", IFN_ZERO_OR_MORE, UNDEF },
	{ "", IFN_NONE, UNDEF }
};
...
int
init_ifn_parse_fdbkinfo(spec, char_op_table)
SPEC *spec;
char *char_op_table;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ifn_parse_fdbkinfo");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ifn_parse_fdbkinfo");
...
int
ifn_parse_fdbkinfo(ifn_query, ifn_node, inst)
IFN_QUERY *ifn_query;
...
int inst;
{
    ...
    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "ifn_parse_fdbkinfo");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering ifn_parse_fdbkinfo");
    ...
    if (same_tag(stop_tag, "FEEDBACK_INFO")) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ifn_parse_fdbkinfo");
    return(retval);
}

int
close_ifn_parse_fdbkinfo(inst)
int inst;
{
    STATIC_INFO *ip;

    if (!VALID_INST(inst)) {
	set_error(SM_ILLPA_ERR, "Instantiation", "close_ifn_parse_fdbkinfo");
	return(UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering close_ifn_parse_fdbkinfo");

    (void) free((char *)ip->kids);
    ip->valid_info = 0;

    PRINT_TRACE (2, print_string, "Trace: leaving close_ifn_parse_fdbkinfo");
    return(0);
}
