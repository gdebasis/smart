#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/info_op_qtext.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Parse SGML-ish <QUOTED_TEXT>
 *1 local.infoneed.op.parse.quoted_text
 *2 ifn_parse_qtext(ifn_query, ifn_node, inst)
 *3   IFN_QUERY *ifn_query;
 *3   IFN_TREE_NODE *ifn_node;
 *3   int inst;
 *4 init_ifn_parse_qtext(spec, unused)
 *5   "index.parse.trace"
 *4 close_ifn_parse_qtext(inst)

 *7 Parse an SGML-ish QUOTED_TEXT, as in TIPSTER DN2.  First process a fixed
 *7 set of possible characteristics in the <QUOTED_TEXT ...> header, then
 *7 call the generic infoneed_parse_data() routine to process child data.
 *7 Allow partial processing of other operators than QUOTED_TEXT, in case
