#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libadvq/infoneed_adv.c,v 11.2 1997/09/17 16:51:18 smart Exp $";
#endif
 ...
 *0 Convert intermediate IFN_TREE_NODE tree to ADV_VEC
 *1 none
 *2 ifn_to_advv(ifn_root, adv_vec, inst)
 *3   IFN_TREE_NODE *ifn_root;
 *3   ADV_VEC *adv_vec;
 *3   int inst;
 *4 init_ifn_to_advv(spec, unused)
 *5   "infoneed.num_ops"
 *5   "index.parse.trace"
 *5   "infoneed.op.*.name"
 *4 close_ifn_to_advv(inst)
 *2 ext2int_docid(buf, docid)
 *3   SM_BUF *buf;
 *3   EID *docid;
 *2 ext2int_coll(unused)
 *3   SM_BUF *unused;

 *7 Convert intermediate IFN_TREE_NODE tree to ADV_VEC.  First look up
 *7 the operator names for this instance, and map the operator numbers
 *7 to a standardized set that covers all supported variants of the
 *7 SGML-like TIPSTER DN2 language, so we will be able to make decisions
 *7 about which IFN_TREE operators are ADV_VEC operators, leaves, etc.
 *7
 *7 Count the number of nodes etc. in the tree and allocate space in
 *7 the ADV_VEC for them.  Then recursively descend the tree, filling in
 *7 the ADV_VEC.  Many IFN_TREE operators need some of their children
 *7 processed breadth-first to get in the right places in the ADV_VEC.
 *7 The remaining parts of the tree are processed normally.
 *7
 *7 After the ADV_VEC is completely formed from the operators etc. of the
 *7 IFN_TREE_NODEs, a feedback routine may modify the ADV_VEC based on
 *7 feedback information also found in the ifn_tree.
 *7
 *7 1 is returned on normal completion, UNDEF if error.
 *7
 *7 ext2int_docid() converts the external form of a SMART docid to the
 *7 internal form.
 *7
 *7 ext2int_coll() will convert the external form of a SMART collection id
 *7 to the internal form, when such a form exists.

 *9 Assumes there's only one DN2 in the IFN_TREE_NODE tree, whereas we
 *9 could have gotten one from each section of the query.
***********************************************************************/
