 ...
 *1 local.retrieve.ctype_coll.ctype_idf
 *2 sim_ctype_idf (qvec, results, inst)
 *3   VEC *qvec;
 *3   RESULT *results;
 *3   int inst;
 *4 init_sim_ctype_idf (spec, param_prefix)
 *5   "retrieve.rank_tr"
 *5   "retrieve.ctype_coll.trace"
 *5   "ctype.*.inv_file"
 *5   "ctype.*.inv_file.rmode"
 *5   "ctype.*.sim_ctype_weight"
 *4 close_sim_ctype_idf (inst)

 *7 This is a version of the work-horse retrieval procedure.  It takes a query
 ...
 *7 name will be "ctype.N.", where N is the ctype for this query.
 *7 The term weights given by the query vector are first multiplied by the
 *7 idf for that term, as given by con_cw_idf().  This is normally used
 *7 only for collections with a length normalization unaffected by term weight.
 ...
 *8 sim_ctype_idf is most often called by retrieve.retrieve.inverted()
***********************************************************************/
...
int
init_sim_ctype_idf (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_idf");
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_idf");
        return (UNDEF);
    }
    ...
    if (UNDEF == (ip->idf_inst = init_con_cw_idf (spec, param_prefix))) {
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_idf");
    return (new_inst);
}

int
sim_ctype_idf (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_idf");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_idf");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    for ( query_con = new_qvec.con_wtp; 
        ...
        /* Weight query term by ctype weight and by idf.  */
        /* Skip this term if new query */
        /* weight is 0 */
        if (UNDEF == con_cw_idf (&query_con->con, &idf, ip->idf_inst))
            return (UNDEF);
        if (0.0 == (query_wt = ip->ctype_weight * idf * query_con->wt)) {
            continue;
        }
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_idf");
                    return (UNDEF);
                }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_idf");
    return (1);
}


int
close_sim_ctype_idf (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_idf");
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ctype_idf");
        return (UNDEF);
    }
    ip  = &info[inst];
    ip->valid_info--;
    ...
    if (ip->valid_info == 0) {
        ...
        if (UNDEF == close_inv (ip->inv_fd))
            return (UNDEF);
        if (UNDEF == close_con_cw_idf (ip->idf_inst))
            return (UNDEF);
    }

    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_idf");
    return (0);
}
