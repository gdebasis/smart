#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libindexing/sentsim_trlm_phrase.c,v 11.2 1993/02/03 16:51:11 smart Exp $";
#endif

/* Copyright (c) 1991, 1990, 1984 - Gerard Salton, Chris Buckley. 
 *
 * Compute the TRLM similarity of every sentence with the query by:
 * 1) Computing the TRLM similarity of every term (ctype 0)
 * 2) Computing the RLM similarity of every higher ctypes (typically phrases)
 * 3) Aggregating over these two...
...
static SPEC_PARAM spec_args[] = {
    ...
    {"sentsim_trlm.vecs_vecs.vec_vec", getspec_func, (char *) &vec_vec_ptab},
    {"sentsim_trlm.mixing_lambda", getspec_float, (char*)&mixing_lambda},
    {"sentsim_trlm.rerank_lambda", getspec_float, (char*)&rerank_lambda},
    {"sentsim_trlm.multifaceted", getspec_bool, (char *) &multifaceted},
    {"sentsim_trlm.one_topic", getspec_bool, (char *) &oneTopic},
    {"sentsim_trlm.vecs_vecs.vec_vec", getspec_func, (char *) &vec_vec_ptab},
    {"sentsim_trlm.alpha", getspec_float, (char *) &alpha},
...
int init_sentsim_trlm_phrase (SPEC* spec, char* unused)
{
	int ctype;
	DIR_ARRAY dir_array;
	char conceptName[64];

    PRINT_TRACE (2, print_string, "Trace: entering init_sentsim_trlm_phrase");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sentsim_trlm_phrase");
    return 0;
}
...
int sentsim_trlm_phrase (Sentence* sentences, int numSentences, VEC* qvec, TR_VEC* tr)
{
	TR_TUP  *tr_tup;
	int     ctype;
	long    tr_ind, con, i;
	...
	Sentence* s;
	float   p_w_D, p_D_Q, kl_div, docLen, norm_tf, totalSim = 0;
	VEC_PAIR  vec_pair;

    PRINT_TRACE (2, print_string, "Trace: entering sentsim_trlm_phrase");
	...
	for (tr_ind = 0; tr_ind < tr->num_tr; tr_ind++) {
		...
		p_D_Q = !multifaceted? tr_tup->sim : getMarginalizedQueryGenEstimate(&ldamodel, tr_ind,
		...
		for (ctype = 0, start = dvec.con_wtp; ctype < dvec.num_ctype; ctype++) {
			...
			for (conwtp = start; conwtp < &start[dvec.ctype_len[ctype]]; conwtp++) {
				...
				if (conwtp->con >= 0 && conwtp->con < collstats_info[ctype].num_freq) { // sanity check of array index access
					if (ctype == 0) {
						// LDA estimation has only been done for the terms i.e. ctype 0s
						if (!oneTopic)
							p_w_D = mixing_lambda*compute_marginalized_P_w_D(&ldamodel, conwtp, tr_ind) +
								(1-mixing_lambda)*collstats_info[ctype].freq[conwtp->con]/collstats_info[ctype].totalDocFreq;
						else
							p_w_D = mixing_lambda*per_topic_p_w_d(&ldamodel, conwtp, tr_ind) +
								(1-mixing_lambda)*collstats_info[ctype].freq[conwtp->con]/collstats_info[ctype].totalDocFreq;
...
#define AGGR
	...
	for (s = sentences; s < &sentences[numSentences]; s++) {
		svec = &(s->svec);
		s->sim = 0;
		docLen = (float)getDocumentLength(svec, 0);
		...
		for (ctype = 0, start = svec->con_wtp; ctype < svec->num_ctype; ctype++) {
			for (conwtp = start; conwtp < &start[svec->ctype_len[ctype]]; conwtp++) {
				if (conwtp->con >= 0 && conwtp->con < collstats_info[ctype].num_freq) { // sanity check of array index access
					// P(s|R) = \prod_{w \in s} P(w|R)
					s->sim += (1-alpha)*collstats_info[ctype].p_w_Q[conwtp->con];
				}
			}
	...
	for (s = sentences; s < &sentences[numSentences]; s++) {
		...
		for (ctype = 0, start = svec->con_wtp; ctype < svec->num_ctype; ctype++) {
			...
			for (conwtp = collstats_info[ctype].p_w_R; conwtp < &(collstats_info[ctype].p_w_R[collstats_info[ctype].vsize]); conwtp++) {
				...
				p_w_D = rerank_lambda*norm_tf + (1-rerank_lambda)*collstats_info[ctype].freq[conwtp->con]/collstats_info[ctype].totalDocFreq;
				kl_div += conwtp->wt * log(conwtp->wt/p_w_D);
				///kl_div += p_w_D * log(p_w_D/conwtp->wt);
			}
			start += svec->ctype_len[ctype];
		}
		s->sim += (1-alpha)*1/kl_div;
	}
#endif

	close_lda_est_inst(&ldamodel);
    PRINT_TRACE (2, print_string, "Trace: leaving sentsim_trlm_phrase");
    return (1);
}

int close_sentsim_trlm_phrase ()
{
	int ctype;
    PRINT_TRACE (2, print_string, "Trace: entering close_sentsim_trlm_phrase");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_sentsim_trlm_phrase");
    return (0);
}
