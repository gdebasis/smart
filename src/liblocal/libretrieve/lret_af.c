 ...
 *1 local.retrieve.top.learn_ret_af
 *2 learn_ret_af (unused1, unused2, inst)
 *3   char *unused1;
 *3   char *unused2;
 *3   int inst;
 *4 init_learn_ret_af (spec, unused)
...
static SPEC_PARAM learn_spec_args[] = {
...
static int num_learn_spec_args = sizeof (learn_spec_args) /
         sizeof (learn_spec_args[0]);
...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_learn_ret_af");
    ...
    /* Setup all initialization procedures */
    ...
    /* Initialize learning segment */
    if (UNDEF == lookup_spec (spec, &learn_spec_args[0], num_learn_spec_args))
        return (UNDEF);
    ...
    ip->valid_info++;
    PRINT_TRACE (2, print_string, "Trace: leaving init_learn_ret_af");
    return (new_inst);
}

int
learn_ret_af (unused1, unused2, inst)
char *unused1;
char *unused2;
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering learn_ret_af");

    if (UNDEF == recursive_pass ((long)0))
        return (UNDEF);

    PRINT_TRACE (2, print_string, "Trace: leaving learn_ret_af");
    return (1);
}
...
do_work ()
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering pass of ret_af");
    ...
    if (UNDEF == (ip->get_query.inst =
                  get_query_ptab->init_proc (current_spec, NULL)) ||
        UNDEF == (ip->get_seen_docs.inst =
                  get_seen_docs_ptab->init_proc (current_spec, NULL)) ||
        UNDEF == (ip->coll_sim_pass1.inst =
                  coll_sim_ptab->init_proc (current_spec, "pass1.")) ||
        UNDEF == (ip->coll_sim_pass2.inst =
                  coll_sim_ptab->init_proc (current_spec, "pass2.")) ||
/*         UNDEF == (ip->store_results.inst = */
/*                   store_results_ptab->init_proc (current_spec, NULL)) || */
        UNDEF == (ip->pfeedback.inst = 
                  pfeedback_ptab->init_proc (current_spec, NULL))) {
        return (UNDEF);
    }
    ...
    while (1 == (status = ip->get_query.ptab->proc (NULL,
        ...
        /* Evaluate and store query results */
        tr_vec.num_tr = 0;
...
int
close_learn_ret_af (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_learn_ret_af");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_learn_ret_af");
    return (0);
}
...
init_output_result()
{
    ...
    if (NULL == (rh = get_rel_header (query_file)))
        return (UNDEF);
    ...
    if (NULL == (output_fd = fopen (learn_output_file, "w")))
        return (UNDEF);
    fprintf (output_fd, "number_params: %ld, num_wanted %ld, 8 precision cutoffs\nParam_name\t\tinit_value\tincrement\tnum_increments\n",
             num_passes, num_wanted);
    for (i = 0; i < num_passes; i++) {
        fprintf (output_fd, "%s\t%f\t%f\t%ld\n",
             learn_param[i],
             learn_init_value[i],
             learn_increment[i],
             learn_num_increment[i]);
    }
    ...
    fprintf (output_fd,":: av_prec :: cutoff_prec_at %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld\n",
             rank_cutoff[7],
             rank_cutoff[6],
             rank_cutoff[5],
             rank_cutoff[4],
...
TR_VEC *tr_vec;
{
    ...
    RR_VEC rr_vec;         /* Used to get num rel for this query */
    ...
    /* Use the actual number of relevant docs for query, if available */
    if (ip->qrels_fd != UNDEF) {
        rr_vec.qid = tr_vec->qid;
        if (1 == seek_rr_vec (ip->qrels_fd, &rr_vec) &&
            1 == read_rr_vec (ip->qrels_fd, &rr_vec)) {
            total_num_rel = rr_vec.num_rr;
        }
    }
