 ...
 *7 Only ctype 0 terms considered during reranking.
***********************************************************************/
...
char *unused;
{
    ...
    fprintf(stderr, "q_tr_coocc.sinonly.c -- only ctype 0 terms handled\n");
...
float *sim;
{
	...
	if (dvec.con_wtp[i].con < query->con_wtp[j].con)
	    ...
	    matchlist[num_match].dwt = dvec.con_wtp[i].wt;
	...
	for (j = 0; j < i; j++) {
		...
		if (this_query[k].con1 == coni && 
		    this_query[k].ctype1 == 0 && 
		    this_query[k].con2 == conj &&
		    this_query[k].ctype2 == 0) {
...
{
    int status;
    long num_match, coni, conj, sec_no, begin_window,end_window, tf, i, j, k;
	...
	while (begin_window == 0 ||
		...
		if (this_sect->concepts[i].con < query->con_wtp[j].con)
		    ...
		    for (tf = 0; 
			 i < end_window &&
			 this_sect->concepts[i].con == query->con_wtp[j].con;
			 i++, tf++);
