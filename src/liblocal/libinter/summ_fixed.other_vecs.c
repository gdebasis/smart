...
typedef struct {
    ...
    int lcom_parse_inst, eid_inst, comp_inst, seg_inst, show_inst, vv_inst,
	cent_inst; 
...
char *unused;
{
	...
	UNDEF==(ip->cent_inst = init_create_cent(spec, NULL)))
	return(UNDEF);
...
int inst;
{
	...
	UNDEF == close_create_cent(ip->cent_inst))
	return(UNDEF);
...
int inst;
{
	...
	sprintf(char_buf, "%s/%d.%ld", summary_dir, uid, docid);
	    ...
	    set_error (SM_ILLPA_ERR, "Summary file", "seg_path_core");
	    return (UNDEF);
	}
...
int inst;
{
    ...
    /* Initialize path. */
    path_len = (int) ceil(DEFAULT_PERCENT * segres.num_nodes/100.0);
...
INTER_STATE *is;
{
    ...
    EID_LIST sent_list, other_paras;
    VEC other_vec;
    VEC_LIST svec, ovec;
    VEC_LIST_PAIR pair;
    ALT_RESULTS curr_sent;

    if (NULL == (other_paras.eid = Malloc(ip->num_nodes, EID)))
	return(UNDEF);
	...
	sprintf(did_buf, "%ld.p%d.s", path->eid[i].id, path->eid[i].ext.num);
	...
	/* Get vector for all other paras in document */
	for (j = 0, other_paras.num_eids = 0; j < ip->num_nodes; j++)
	    if (!EID_EQUAL(ip->nodes[j].node_id, path->eid[i]))
		other_paras.eid[other_paras.num_eids++] = ip->nodes[j].node_id;
	if (UNDEF == create_cent(&other_paras, &other_vec, ip->cent_inst))
	    return(UNDEF);

	/* Compute similarity of each sentence to every other para */
	svec.num_vec = sent_list.num_eids; svec.vec = ip->sent_vecs;
	ovec.num_vec = 1; ovec.vec = &other_vec;
