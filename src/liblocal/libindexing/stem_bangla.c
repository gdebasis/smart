...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_stem_bangla");
    ...
    if (UNDEF == fseek(stem_fp, 0, SEEK_END) ||
        ...
        set_error(errno, "Problem determining file size", "init_stem_bangla");
        return UNDEF;
    }
    rewind(stem_fp);

    if (NULL == (inbuf = Malloc(filelen, char))) {
        set_error(errno, "Out of memory", "init_stem_bangla");
        return UNDEF;
    }
    if (filelen != fread(inbuf, sizeof(char), filelen, stem_fp)) {
        set_error(errno, "Problem reading file", "init_stem_bangla");
        return UNDEF;
    }
    fclose(stem_fp);
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_stem_bangla");
    return (0);
}
...
int inst;
{
    PRINT_TRACE (2,print_string, "Trace: entering close_stem_bangla");
    ...
    PRINT_TRACE (2,print_string, "Trace: leaving close_stem_bangla");
    return (0);
}
