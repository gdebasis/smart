...
/* This version of pp_xml.c was used for the runs officially submitted to
 * INEX 2007.
 */
...
STATIC_INFO *ip;
{
    ...
    while (top >= 0) {
        ...
        if (cur_node->type == XML_TEXT_NODE &&
            ...
            if (status != 0) { // some non-whitespace chars found
                ...
                /* HACK! HACK! HACK ! */
                ip->mem_disp[ip->num_sections - 1].subsect = (SM_DISP_SEC *)
                ...
                if (ip->num_nodes == ip->max_nodes) {
                    ...
                    if (NULL == (ip->nodelist = Realloc(ip->nodelist, 
                                                        ip->max_nodes,
                                                        xmlNodePtr)) ||
                        NULL == (ip->supplist = Realloc(ip->supplist, 
                                                        ip->max_nodes,
                                                        SUPPLEMENTARY)))
                        return(UNDEF);
                }
    ...
    printf("\n\n=======================================================\n\n"
           "Interesting nodes:\n\n");
    for (num_nodes = 0; num_nodes < ip->num_nodes; num_nodes++) {
        int section_number = (int) ip->nodelist[num_nodes]->_private;
        SM_DISP_SEC *sec_ptr = ip->mem_disp + section_number;

        ptr = xmlGetNodePath(ip->nodelist[num_nodes]);
        printf("%s:\n", ptr);
        xmlFree(ptr);
        for (ptr = ip->buf.buf + sec_ptr->begin_section; 
             ptr < ip->buf.buf + sec_ptr->end_section;
             ptr++) 
            putchar(*ptr);
        printf("\n\n--------------------------------------------------\n\n");
    }
