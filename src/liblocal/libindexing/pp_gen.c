 ...
 *1 local.index.preparse.named_doc
 *1 local.index.preparse.next_doc
 *2 pp_named_doc(doc_id, output_doc, inst)
 *3   EID *doc_id;
 *3   SM_INDEX_TEXTDOC *output_doc;
 *3   int inst;
 *2 pp_next_doc(unused, output_doc, inst)
 *3   char *unused;
 *3   SM_INDEX_TEXTDOC *output_doc;
 *3   int inst;
 *4 init_pp_named_doc(spec_ptr, qd_prefix)
 *4 init_pp_next_doc(spec_ptr, qd_prefix)
 *5    "index.preparse.trace"
 *5    "*.named_get_text"
 *5    "*.next_get_text"
 *5    "*.preparser"
 *4 close_pp_named_doc(inst)
 *4 close_pp_next_doc(inst)
 ...
 *8 Difference between this and the regular pp_next_text is that when the
 *8 actual preparser is called (c. line 270), it is supplied the entire
 *8 output_doc, instead of output_doc->mem_doc only. This allows the preparser
 *8 to change output_doc->doc_text, the character buffer that actually holds
 *8 the text to be indexed (offsets in output_doc->mem_doc point into this
 *8 buffer). This is necessary when parsing XML documents, because the libxml2
 *8 parser currently does not provide information about byte offsets, so the
 *8 useful content is actually copied into a separate buffer, and the offsets
 *8 in output_doc->mem_doc point into this new buffer.

 *8 pp_named_doc also changed as above.
 *8 For use with pp_xml.c (qv).
 ***********************************************************************/
...
int
init_pp_named_doc(spec, qd_prefix)
SPEC *spec;
char *qd_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_pp_named_doc");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_pp_named_doc");
...
int
init_pp_next_doc(spec, qd_prefix)
SPEC *spec;
char *qd_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_pp_next_doc");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_pp_next_doc");
...
int
pp_named_doc(doc_id, output_doc, inst)
EID *doc_id;
...
int inst;
{
    ...
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "pp_named_doc");
        return (UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering pp_named_doc");
    ...
    status = ip->pp_proc.ptab->proc(&(ip->textbuf), output_doc,
					ip->pp_proc.inst);
    if (UNDEF == status)
        return (UNDEF);
    else if (0 == status) {
...
int
pp_next_doc(unused, output_doc, inst)
char *unused;
SM_INDEX_TEXTDOC *output_doc;
int inst;
{
    ...
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "pp_next_doc");
        return (UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering pp_next_doc");
    ...
    if (UNDEF == status)
        ...
        PRINT_TRACE (2, print_string, "Trace: leaving pp_next_doc");
        return (0);
    }
    ...
    /* preparse text, setting up SM_DISPLAY part of output_doc */
    PRINT_TRACE(1, print_string, output_doc->mem_doc.file_name);
    status = ip->pp_proc.ptab->proc(&(ip->textbuf), output_doc,
					ip->pp_proc.inst);
    if (UNDEF == status)
        return (UNDEF);
...
int
close_pp_named_doc(inst)
int inst;
{
    STATIC_INFO *ip;

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_pp_named_doc");
        return (UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering close_pp_named_doc");
    ip->valid_info--;
    if (0 == ip->valid_info) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_pp_named_doc");

    return (0);
}

int
close_pp_next_doc(inst)
int inst;
{
    STATIC_INFO *ip;

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_pp_next_doc");
        return (UNDEF);
    }
    ip = &info[inst];

    PRINT_TRACE (2, print_string, "Trace: entering close_pp_next_doc");
    ip->valid_info--;
    if (0 == ip->valid_info) {
	if (UNDEF == ip->textproc.ptab->close_proc(ip->textproc.inst))
	    return(UNDEF);
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_pp_next_doc");
