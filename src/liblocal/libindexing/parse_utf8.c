#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libindexing/parse_utf8.c,v 11.2 1993/02/03 16:51:11 smart Exp $";
#endif
 ...
 *1 local.index.parse_sect.parse_utf8
 *2 parse_utf8 (token, consect, inst)
 *3   SM_TOKENSECT *token;
 *3   SM_CONSECT *consect;
 *3   int inst;
 *4 init_parse_utf8(spec, unused)
 ...
 *4 close_parse_utf8(inst)

 *7 Adapted from parse_phrase.c to work with token_utf8.
 *7 Since normalization to lower case depends on wchar info, this is taken care
 *7 of in the tokenizer (token_utf8).
 *7 Convert to concept, and assign appropriate ctype.
...
int 
init_parse_utf8(spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_parse_utf8");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_parse_utf8");

    return (new_inst);
}

int
parse_utf8 (tokensect, consect, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering parse_utf8");
    PRINT_TRACE (6, print_tokensect, tokensect);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "parse_utf8");
        return (UNDEF);
    }
    ip  = &info[inst];

    /* Reserve space for conlocs.  Note, in parse_utf8 (as opposed to 
       non-phrase methods) we are not guaranteed to have less conlocs than
       tokens, so reserve double the space */
    ...
    for (i = 0; i < tokensect->num_tokens; i++) {
        ...
        first_capital = token->tokentype & SM_INDEX_MIXED_FIRST_CAP;
        other_capital = token->tokentype & SM_INDEX_MIXED_OTHER_CAP;
        token->tokentype &= ~SM_INDEX_MIXED_FIRST_CAP & ~SM_INDEX_MIXED_OTHER_CAP;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving parse_utf8");

    return (1);
}

int 
close_parse_utf8(inst)
int inst;
{
    STATIC_INFO *ip;
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_parse_utf8");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_parse_utf8");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_parse_utf8");
                                ...
                                add phrases */
{
    ...
    PRINT_TRACE (8, print_string, "Trace: entering parse_utf8.add_con");
    ...
    /* Phrasing only of non-stopwords */
    if (! rec_call) {
        if (token_num - ip->last_comp_token_num <= 2) {
            /* Add phrase */
	    (void) strcpy (phrase, ip->last_comp);
	    ptr = &phrase[strlen(phrase)];
	    *ptr++ = ' ';
	    (void) strcpy (ptr, token);
            if (UNDEF == add_con (ip,
                                  phrase,
                                  &ip->parse_func[SM_PT_F_PHRASE],
                                  token_num,
