#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libindexing/token_hi.c,v 11.2 1993/02/03 16:51:18 smart Exp $";
#endif
 ...
 *1 local.index.token_sect.token_hi
 *2 token_hi (pp_buf, t_sect, inst)
 *3   SM_BUF *pp_buf;
 *3   SM_TOKENSECT *t_sect;
 *3   int inst;
 *4 init_token_hi (spec, unused)
 *5   "*.interior_char"
 *5   "*.interior_hyphenation"
 *5   "*.endline_hyphenation"
 *5   "index.token_sect.trace"
 *4 close_token_hi (inst)
...
int
init_token_hi (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    if (UNDEF == lookup_spec_prefix (spec,
        ...
        return (UNDEF);
    PRINT_TRACE (2, print_string, "Trace: entering init_token_hi");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_token_hi");
...
{                                                                             \
   while (ptr < end_ptr) {                                                    \
       if (UNDEF == (num_converted = mbtowc(&wc, ptr, MB_CUR_MAX))) {         \
           print_error("token_hi", "valid multi-byte character not found"); \
           ptr++;                                                             \
           continue;                                                          \
       }                                                                      \
       if (ishialpha(wc) || ishiintern(wc)) {                                 \
           /* copy this character */                                          \
           for (i = 0; i < num_converted && buf_ptr + 1 < end_buf; i++)       \
               *buf_ptr++ = *ptr++;                                           \
       }                                                                      \
       else if (ishidigit(wc)) {                                              \
           mixed_token = 1;                                                   \
           for (i = 0; i < num_converted && buf_ptr + 1 < end_buf; i++)       \
               *buf_ptr++ = *ptr++;                                           \
       }                                                                      \
                ...
                (ishialpha(next) || ishidigit(next) || ishiintern(next)))   { \
           mixed_token = 1;                                                   \
           *buf_ptr++ = *ptr++;                                               \
       }                                                                      \
       else if (*ptr == '-') {                                                \
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering token_hi");
   ...
   /* Determine maximum size of buffer needed to hold all the document's
    ...
    if (size_needed > ip->size_token_buf) {
        ...
        if (NULL == (ip->token_buf = malloc(ip->size_token_buf))) {
            set_error (errno, "Out of space", "token_hi");
            return (UNDEF);
        }
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving token_hi");
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering close_token_hi");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_token_hi");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_token_hi");
