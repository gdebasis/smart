...
static SPEC_PARAM pp[] = {
    {"post_pp.init_size", getspec_long, (char *)&init_size},
...
int inst;
{
    long num_sec, curr_size, sec_size, i;
    STATIC_INFO *ip;
    ...
    /* Duplicate initial init_size bytes */
    for (curr_size = 0, i = 0; curr_size < init_size && i < num_sec; i++) {
	...
	ip->mem_disp[num_sec + i].end_section =
	    input_doc->mem_doc.sections[i].begin_section + 
	    MIN(sec_size, init_size - curr_size);
	curr_size += MIN(sec_size, init_size - curr_size);
