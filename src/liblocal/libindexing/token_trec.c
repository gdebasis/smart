#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libindexing/token_trec.c,v 11.2 1993/02/03 16:51:18 smart Exp $";
#endif
 ...
 *1 local.index.token_sect.token_trec
 *2 token_trec (pp_buf, t_sect, inst)
 *3   SM_BUF *pp_buf;
 *3   SM_TOKENSECT *t_sect;
 *3   int inst;
 *4 init_token_trec (spec, unused)
 *5   "*.interior_char"
 *5   "*.interior_hyphenation"
 *5   "*.endline_hyphenation"
 *5   "index.token_sect.trace"
 *4 close_token_trec (inst)
 ...
 *7
 *7 Differs from token_sect in that SGML constructs are explicitly
 *7 ignored.  All tokens (inclusive) between < > are ignored, as are
 *7 all tokens beginning with a '&'.  This is a hack; it should be
...
int
init_token_trec (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    if (UNDEF == lookup_spec_prefix (spec,
        ...
        return (UNDEF);
    PRINT_TRACE (2, print_string, "Trace: entering init_token_trec");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_token_trec");
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering token_trec");
   ...
   /* Determine maximum size of buffer needed to hold all the document's
    ...
    if (size_needed > ip->size_token_buf) {
        ...
        if (NULL == (ip->token_buf = malloc ((unsigned) ip->size_token_buf))) {
            set_error (errno, "Out of space", "token_trec");
            return (UNDEF);
        }
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving token_trec");

    return (0);
}

int
close_token_trec (inst)
int inst;
{
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_token_trec");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_token_trec");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_token_trec");
