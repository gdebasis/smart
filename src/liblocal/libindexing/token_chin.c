 ...
 *1 index.token_sect.token_chinese
 *2 token_chinese (pp_buf, t_sect, inst)
 *3   SM_BUF *pp_buf;
 *3   SM_TOKENSECT *t_sect;
 *3   int inst;
 *4 init_token_chinese (spec, unused)
 *5   "*.interior_char"
 *5   "*.interior_hyphenation"
 *5   "*.endline_hyphenation"
 *5   "index.token_sect.trace"
 *4 close_token_chinese (inst)

 *7 Take the preparsed section pp_buf, and construct a list of tokens 
 *7 and their types in it, returning the list in t_sect.  
 *7 
 *7 In English sections, interior_char
 ...
 *7 precedent.
 *7
 *7 All non-ascii characters are treated as being part of a Chinese character.
 *7 They are paired to form a token.
 *7 
 ...
 *7
 *7 Differs from the default token_sect in that SGML constructs are explicitly
 *7 ignored.  All tokens (inclusive) between < > are ignored, as are
 *7 all tokens beginning with a '&'.  This may be a hack; perhaps it should be
 *7 handled at the preparse level.
 *7
...
/* The last 128 (non-ascii) characters in sm_ctype are assumed to be
   Chinese. MORE ATTENTION NEEDED */
...
int
init_token_chinese (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    if (UNDEF == lookup_spec_prefix (spec,
        ...
        return (UNDEF);
    PRINT_TRACE (2, print_string, "Trace: entering init_token_chinese");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_token_chinese");
...
int
token_chinese (pp_buf, t_sect, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering token_chinese");
   ...
   /* Determine maximum size of buffer needed to hold all the document's
    ...
    if (size_needed > ip->size_token_buf) {
        ...
        if (NULL == (ip->token_buf = malloc ((unsigned) ip->size_token_buf))) {
            set_error (errno, "Out of space", "token_chinese");
            return (UNDEF);
        }
    }
    ...
    while (ptr < end_ptr) {
        ...
        switch (ip->char_type[*ptr]) {
          ...
          case T_SGMLT:
            ...
            break;
          case T_C:
            /* Chinese characters. Pair up to form word */
            /* Those starting with \241 are punctuation and other symbols*/
            /* Those starting with \243 are punctuation, digits, letters */
            if (*ptr == (unsigned char) '\241' ||
                *ptr == (unsigned char) '\243') {
                *buf_ptr++ = *ptr++; 
                if (ptr < end_ptr)
                    *buf_ptr++ = *ptr++;
                t_sect->tokens[t_sect->num_tokens].tokentype = SM_INDEX_PUNC;
            ...
            *buf_ptr++ = *ptr++;
            if (ptr >= end_ptr || ip->char_type[*ptr] != T_C) {
                PRINT_TRACE (1, print_string, "Non-ascii,non-Chinese char");
                PRINT_TRACE (1, print_string, ptr-1);
                t_sect->tokens[t_sect->num_tokens].tokentype = SM_INDEX_MIXED;
            }
            else {
                *buf_ptr++ = *ptr++;
                t_sect->tokens[t_sect->num_tokens].tokentype = SM_INDEX_LOWER;
            }
            break;
          case T_CN:
          default:
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving token_chinese");

    return (0);
}

int
close_token_chinese (inst)
int inst;
{
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_token_chinese");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_token_chinese");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_token_chinese");
