 ...
 * The new file pp_window (in contrast to the old pp_window_old)
 * takes a preparsed document instead of a TEXTLOC object. Hence
 * it does not need to deal with the preparsing of a document.
 * The interface of pp_window is thus identical to pp_pp_parts
 * which allows pp_window to be called from get_textdoc.
 *  The new version also has introduces a configuration parameter
 *  to control overlapping of windows.
 *  (Note: In SBQE, we don't require overlapping windows).
 *
 *0 pre-parser front-end that constructs overlapping text windows
 *1 local.index.parts_preparse.window
 *2 pp_window (in_pp, out_pp, inst)
 *3   SM_INDEX_TEXTDOC *in_pp;
 *3   SM_INDEX_TEXTDOC *out_pp;
 *3   int inst;
 *4 init_pp_window (spec_ptr, unused)
 *5   "index.parts_preparse.trace"
 *5   "parts_preparse.preparse_window_size"
 *5   "pp_parts.named_preparse"
 *5   "pp_parts.next_preparse"
...
static SPEC_PARAM spec_args[] = {
    {"pp_window.window_size", getspec_long,(char *)&window_size},
    {"pp_window.overlap", getspec_bool,(char *)&overlap},
    TRACE_PARAM ("pp_window.trace")
    };
...
int inst;
{
    ...
    if (UNDEF == pp_pp_window (ip, in_pp, out_pp))
        return (UNDEF);
...
SM_INDEX_TEXTDOC *out_pp;
{
    ...
    ip->num_sections = 0;
    for (i = 0; i < in_pp->mem_doc.num_sections; i++) {
		if (UNDEF == parse_section (ip, &in_pp->mem_doc.sections[i], in_pp->doc_text))
			return (UNDEF);
    }
