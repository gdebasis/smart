#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libindexing/token_utf8.c,v 11.2 1993/02/03 16:51:18 smart Exp $";
#endif
 ...
 *1 local.index.token_sect.token_utf8
 *2 token_utf8 (pp_buf, t_sect, inst)
 *3   SM_BUF *pp_buf;
 *3   SM_TOKENSECT *t_sect;
 *3   int inst;
 *4 init_token_utf8 (spec, unused)
 *5   "*.interior_char"
 *5   "*.interior_hyphenation"
 *5   "*.endline_hyphenation"
 *5   "index.token_sect.trace"
 *4 close_token_utf8 (inst)

 *7 Adapted from token_trec to handle UTF8 input.
 ...
 *9 Assumes that all characters besides uppercase and lowercase letters
 *9 (e.g. digits, punctuation, etc.) are plain ASCII.
...
static SPEC_PARAM spec_args[] = {
    ...
    {"index.token_sect.locale", getspec_string, (char *) &locale},
};
...
int
init_token_utf8 (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    if (UNDEF == lookup_spec_prefix (spec,
        ...
        return (UNDEF);
    PRINT_TRACE (2, print_string, "Trace: entering init_token_utf8");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_token_utf8");

    return (new_inst);
}

#define BENG_DARI 0x09f7 

#define CONSUME_TOKEN()                                                       \
{                                                                             \
   while (ptr < end_ptr) {                                                    \
       if (UNDEF == (num_converted = mbtowc(&wc, ptr, MB_CUR_MAX))) {         \
           print_error("token_utf8", "valid multi-byte character not found"); \
           ptr++;                                                             \
           continue;                                                          \
       }                                                                      \
       if (/*iswlower(wc)*/ iswgraph(wc) && wc != BENG_DARI) {                \
           /* copy this character */                                          \
           for (i = 0; i < num_converted && buf_ptr + 1 < end_buf; i++)       \
               *buf_ptr++ = *ptr++;                                           \
       }                                                                      \
       else if (iswupper(wc)) {                                               \
           mixed_token = other_capital = 1;                                   \
           wc = towlower(wc);                                                 \
           if (buf_ptr + MB_CUR_MAX + 1 < end_buf) {                          \
               if (UNDEF == (num_written = wctomb(buf_ptr, wc)))              \
                   print_error("token_utf8", "invalid wide character");       \
               else                                                           \
                   buf_ptr += num_written;                                    \
           }                                                                  \
           ptr += num_converted;                                              \
           ...
           mixed_token = 1;                                                   \
           *buf_ptr++ = *ptr++; /* digits are plain ASCII */                  \
       }                                                                      \
       else if (isascii(*ptr) &&                                              \
                ip->char_type[*ptr] == T_I &&                                 \
                ptr+1 < end_ptr &&                                            \
                (UNDEF != mbtowc(&next, ptr+1, MB_CUR_MAX)) &&                \
                (iswupper(next) || iswlower(next) || iswdigit(next))) {       \
           mixed_token = 1;                                                   \
           *buf_ptr++ = *ptr++;                                               \
       }                                                                      \
       else if (*ptr == '-') {                                                \
   ...
   if (first_capital)                                                         \
       t_sect->tokens[t_sect->num_tokens].tokentype |=                        \
           SM_INDEX_MIXED_FIRST_CAP;                                          \
   if (other_capital)                                                         \
       t_sect->tokens[t_sect->num_tokens].tokentype |=                        \
           SM_INDEX_MIXED_OTHER_CAP;                                          \
   *buf_ptr++ = '\0';                                                         \
   t_sect->num_tokens++;                                                      \
}

int
token_utf8 (pp_buf, t_sect, inst)
...
int inst;
{
    ...
    wchar_t wc, next;
    int mixed_token, first_capital, other_capital, mixed_newline = 0;
    int num_converted, num_written, i;

    PRINT_TRACE (2, print_string, "Trace: entering token_utf8");
   ...
   /* Determine maximum size of buffer needed to hold all the document's
    ...
    if (size_needed > ip->size_token_buf) {
        ...
        if (NULL == (ip->token_buf = malloc(ip->size_token_buf))) {
            set_error (errno, "Out of space", "token_utf8");
            return (UNDEF);
        }
    }
    ...
    while (ptr < end_ptr) {
        ...
        mixed_token = first_capital = other_capital = 0;
        if (UNDEF == (num_converted = mbtowc(&wc, ptr, MB_CUR_MAX))) {
            print_error("token_utf8", "valid multi-byte character not found");
            ptr++;
            continue;
        ...
        else if (/*iswlower(wc)*/ iswgraph(wc) ) {
            /* copy this character */
            for (i = 0; i < num_converted && buf_ptr + 1 < end_buf; i++)
                *buf_ptr++ = *ptr++;
            CONSUME_TOKEN();
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving token_utf8");

    return (0);
}

int
close_token_utf8 (inst)
int inst;
{
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_token_utf8");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_token_utf8");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_token_utf8");
