 ...
 *1 local.ocr.expand.trie_3
 *2 vec_exp_trie_3 (vector, exp_vector, inst)
 *3 VEC *vector;
 *3 VEC *exp_vector;
 *3 int inst;
 *4 init_vec_exp_trie_3 (spec, unused)
...
int
init_vec_exp_trie_3 (spec, unused)
SPEC *spec;
char *unused;
{
    long i;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_vec_exp_trie_3");
    ...
    (void) sprintf (temp_buf, "vec_exp_trie_3.ctype.%ld.", major_ctype);
    ...
    for (i = 0; i < num_exp_method; i++) {
        (void) sprintf (temp_buf, "vec_exp_trie_3.expand.%ld.", i);
        if (UNDEF == lookup_spec_prefix (spec,
                                         &prefix_spec_args[0],
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_vec_exp_trie_3");

    return (0);
#else
    set_error (SM_ILLPA_ERR, "Must have MMAP defined", "vec_exp_trie_3");
    return (UNDEF);
#endif
}

int
vec_exp_trie_3 (vec, exp_vec, inst)
VEC *vec;
VEC *exp_vec;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering vec_exp_trie_3");
    ...
    if (current_ctype != major_ctype) {
        set_error (SM_ILLPA_ERR, "Major_ctype illegal", "vec_exp_trie_3");
        return (UNDEF);
    }
    ...
    while (current_conwt < end_conwt) {
        ...
        if (1 != (status = seek_dict_noinfo (dict_fd, &dict)) ||
            ...
            if (status == 0)
                set_error (SM_INCON_ERR, "Con not in dict", "vec_exp_trie_3");
            return (UNDEF);
        }
        ...
        for (i = 0; i < num_exp_method; i++) {
            ...
            if (0 == strcmp (exp_method[i].method, "exact")) {
                ...
                set_error (SM_INCON_ERR, "Illegal method", "vec_exp_trie_3");
                return (UNDEF);
            }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving vec_exp_trie_3");
    return (1);
}

int
close_vec_exp_trie_3 (inst)
int inst;
{
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_vec_exp_trie_3");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_vec_exp_trie_3");
