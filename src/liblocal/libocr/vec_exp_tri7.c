 ...
 *1 local.ocr.expand.trie_7
 *2 vec_exp_trie_7 (vector, exp_vector, inst)
 *3 VEC *vector;
 *3 VEC *exp_vector;
 *3 int inst;
 *4 init_vec_exp_trie_7 (spec, unused)
...
static float weight_factor;
static long thresh_orig_freq;

static SPEC_PARAM spec_args[] = {
    {"vec_exp_trie.trie_file",       getspec_dbfile, (char *) &trie_file},
    {"vec_exp_trie.doc.textloc_file", getspec_dbfile, (char *) &textloc_file},
    ...
    {"vec_exp_trie.weight_factor",   getspec_float,(char *)&weight_factor},
    {"vec_exp_trie.thresh_orig_freq",getspec_long,(char *)&thresh_orig_freq},
...
static long num_docs_in_collection = 0;
...
int trie_near_exact(), trie_near_sub_1(),trie_near_ins_1(),trie_near_del_1(),
    trie_near_prefix(), trie_near_2(), trie_near_vowels(), trie_near_ks();

int
init_vec_exp_trie_7 (spec, unused)
SPEC *spec;
char *unused;
{
    long i;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_vec_exp_trie_7");
    ...
    (void) sprintf (temp_buf, "vec_exp_trie_7.ctype.%ld.", major_ctype);
    ...
    for (i = 0; i < num_exp_method; i++) {
        (void) sprintf (temp_buf, "vec_exp_trie_7.expand.%ld.", i);
        if (UNDEF == lookup_spec_prefix (spec,
                                         &prefix_spec_args[0],
	...
	rh->num_entries <= 0) {
	set_error (SM_ILLPA_ERR, "Cannot determine size of collection",
		   "vec_exp_trie_7");
	return (UNDEF);
    }
    num_docs_in_collection = rh->num_entries;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_vec_exp_trie_7");

    return (0);
#else
    set_error (SM_ILLPA_ERR, "Must have MMAP defined", "vec_exp_trie_7");
    return (UNDEF);
#endif
}

int
vec_exp_trie_7 (vec, exp_vec, inst)
VEC *vec;
VEC *exp_vec;
int inst;
{
    ...
    DICT_ENTRY dict, stem_dict;
    int status;
    long num_conwt, start_conwt;
    long orig_freq, tot_freq;
    float orig_wt;
    char temp_buf[1024];
    char *stemmed_token;
    int no_reweight;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering vec_exp_trie_7");

    if (UNDEF == copy_vec (&orig_vec, vec))
	return (UNDEF);

    exp_vec->id_num = orig_vec.id_num;
    exp_vec->num_conwt = 0;

    /* Expand only major_ctype */
    current_conwt = orig_vec.con_wtp;
    for (current_ctype = 0;
         current_ctype < major_ctype && current_ctype < orig_vec.num_ctype;
         current_ctype++)
        current_conwt += orig_vec.ctype_len[current_ctype];
    if (current_ctype != major_ctype) {
        set_error (SM_ILLPA_ERR, "Major_ctype illegal", "vec_exp_trie_7");
        return (UNDEF);
    }
    ...
    end_conwt = current_conwt + orig_vec.ctype_len[current_ctype];
    ...
    while (current_conwt < end_conwt) {
        ...
        dict.token = NULL;
        if (1 != (status = seek_dict (dict_fd, &dict)) ||
            1 != (status = read_dict (dict_fd, &dict))) {
            if (status == 0)
                set_error (SM_INCON_ERR, "Con not in dict", "vec_exp_trie_7");
            return (UNDEF);
        }
	    ...
	    current_conwt->wt = weight_factor * current_conwt->wt * idf (collstat_ptr[current_conwt->con]);
            current_conwt++;
            continue;
        }
        ...
        for (i = 0; i < num_exp_method; i++) {
            ...
            if (0 == strcmp (exp_method[i].method, "exact")) {
                ...
                stem_dict.con = UNDEF;
                if (1 == (status = seek_dict (dict_fd, &stem_dict)) &&
                    1 == (status = read_dict (dict_fd, &stem_dict)) &&
            ...
            else if (0 == strcmp (exp_method[i].method, "vowels")) {
                if (UNDEF == trie_near_vowels (dict.token,
                                                   root_trie,
                                                   &exp_method[i].trie_result))
                    return (UNDEF);
            }
            else if (0 == strcmp (exp_method[i].method, "ks")) {
                if (UNDEF == trie_near_ks (dict.token,
                                                   root_trie,
                                                   &exp_method[i].trie_result))
                    return (UNDEF);
            }
            else {
                set_error (SM_INCON_ERR, "Illegal method", "vec_exp_trie_7");
                return (UNDEF);
            }
            /* Reserve space for the new concepts to the con_wt list */
	...
	/* Replace original weight depending on whether a good */
	/* match cross languages */
	/* Assign new idf weight to concept (saving old weight) */
	/* Assumes tf weighted concept coming in */
	orig_freq = collstat_ptr[current_conwt->con];
	orig_wt = current_conwt->wt;

	tot_freq = 0;
        for (i = 0; i < num_exp_method; i++) {
            for (j = 0; j < exp_method[i].trie_result.num_matches; j++) {
		tot_freq += collstat_ptr[exp_method[i].trie_result.matches[j]];
	    }
	}

	/* Assign weight for all terms (weight_factor * natural idf) */
	wt_terms = weight_factor * orig_wt * idf(tot_freq + orig_freq);
        for (i = 0; i < num_exp_method; i++) {
            for (j = 0; j < exp_method[i].trie_result.num_matches; j++) {
	...
	if (orig_freq > thresh_orig_freq) {
	    current_conwt->wt = orig_wt * idf (orig_freq);
	}
	else {
	    current_conwt->wt = wt_terms;
	}
    ...
    if (orig_vec.num_ctype - 1 > max_ctype)
        max_ctype = orig_vec.num_ctype - 1;
    ...
    for (current_ctype = 0; current_ctype <= max_ctype; current_ctype++) {
        ...
        if (current_ctype < orig_vec.num_ctype &&
            orig_vec.ctype_len[current_ctype] > 0) {
            (void) bcopy ((char *) current_conwt,
                          (char *) &conwt[num_conwt],
                          orig_vec.ctype_len[current_ctype] * sizeof (CON_WT));
            num_conwt += orig_vec.ctype_len[current_ctype];
            current_conwt += orig_vec.ctype_len[current_ctype];
        }
    ...
    (void) free_vec (&orig_vec);

    PRINT_TRACE (2, print_string, "Trace: leaving vec_exp_trie_7");
    return (1);
}

int
close_vec_exp_trie_7 (inst)
int inst;
{
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_vec_exp_trie_7");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_vec_exp_trie_7");
...
long freq;
{
    if (freq <= 0)
	return (0.0);
    return (log ((double) num_docs_in_collection / freq));
}
