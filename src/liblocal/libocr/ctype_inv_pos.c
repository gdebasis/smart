 ...
 *1 local.ocr.ctype_coll.ctype_inv_pos
 *2 sim_ctype_inv_pos (qvec, results, inst)
 *3   VEC *qvec;
 *3   RESULT *results;
 *3   int inst;
 *4 init_sim_ctype_inv_pos (spec, param_prefix)
 *5   "retrieve.rank_tr"
 *5   "retrieve.ctype_coll.trace"
 *5   "ctype.*.inv_file"
 *5   "ctype.*.inv_file.rmode"
 *5   "ctype.*.sim_ctype_weight"
 *4 close_sim_ctype_inv_pos (inst)
 ...
 *8 sim_ctype_inv_pos is most often called by retrieve.retrieve.inverted()
***********************************************************************/
...
int
init_sim_ctype_inv_pos (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_sim_ctype_inv_pos");
    if (! VALID_FILE (inv_file)) {
        set_error (SM_ILLPA_ERR,"Non-existant inverted file", "sim_ctype_inv_pos");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_sim_ctype_inv_pos");
    return (new_inst);
}

int
sim_ctype_inv_pos (qvec, results, inst)
VEC *qvec;
RESULT *results;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering sim_ctype_inv_pos");
    PRINT_TRACE (6, print_vector, qvec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "sim_ctype_inv_pos");
        return (UNDEF);
    }
    ip  = &info[inst];
            ...
            if (listwt_ptr->list >= results->num_full_results) {
                if (NULL == (results->full_results = (float *)
                        realloc ((char *) results->full_results,
                                 ...
                                 sizeof (float)))) {
                    set_error (errno, "Realloc partial sim", "sim_ctype_inv_pos");
                    return (UNDEF);
                }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving sim_ctype_inv_pos");
    return (1);
}


int
close_sim_ctype_inv_pos (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_sim_ctype_inv_pos");
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ctype_inv_pos");
        return (UNDEF);
    }
    ip  = &info[inst];
    ip->valid_info--;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_sim_ctype_inv_pos");
    return (0);
}
