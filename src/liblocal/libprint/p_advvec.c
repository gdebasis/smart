#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libprint/p_advvec.c,v 11.2 1993/02/03 16:52:16 smart Exp $";
#endif
...
SM_BUF *outbuf;
{
	...
	Q_ANNOT *annot;
	ANNOT_SE_APP *se_app;
	static char *annot_type[] = { "UNKNOWN", "SUBTREE", "LEAF", "",
			"SE_APP_HELP", "SE_APP_EXPL", "", "COMMENT" };
...
"%d:  SE_APP from %ld to %ld for doc %ld.%u.%u from %ld to %ld with wt %f",
				...
				se_app->weight);
		break;
	    case ADV_ANNOT_UNKNOWN:
	    case ADV_ANNOT_SUBTREE_DATA:
	    case ADV_ANNOT_LEAF_DATA:
	    case ADV_ANNOT_SE_APP_HELP:
	    case ADV_ANNOT_SE_APP_EXPL:
	    case ADV_ANNOT_COMMENT:
		(void) sprintf(fmtbuf, "%%d:  type %%s <%%.%lds>",
				annot->offset.end - annot->offset.begin + 1);
		(void) sprintf(linebuf, fmtbuf,
				i, annot_type[annot->annotation_type],
