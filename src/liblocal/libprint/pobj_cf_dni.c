 ...
 *0 print dict_noinfo entries with correct cll freq info.
 *1 local.print.obj.collstat_dict_noinfo
 *2 print_obj_collstat_dict_noinfo (in_file, out_file, inst)
 *3   char *in_file;
 *3   char *out_file;
 *3   int inst;
 *4 init_print_obj_collstat_dict_noinfo (spec, unused)
 ...
 *5   "print.verbose"
 *5   "print.trace"
 *4 close_print_obj_collstat_dict_noinfo (inst)
...
int open_dict_noinfo(), seek_dict_noinfo(), read_dict_noinfo(), close_dict_noinfo();


int
init_print_obj_collstat_dict_noinfo (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_print_obj_collstat_dict_noinfo");

    PRINT_TRACE (2, print_string, "Trace: leaving init_print_obj_collstat_dict_noinfo");
    return (0);
}

int
print_obj_collstat_dict_noinfo (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering print_obj_collstat_dict_noinfo");
    ...
    if (UNDEF == (dict_index = open_dict_noinfo (dict_file, dict_mode)))
        return (UNDEF);

    while (1 == read_dict_noinfo (dict_index, &dict)) {
    ...
    if (UNDEF == close_dir_array (collstat_index) ||
        UNDEF == close_dict_noinfo (dict_index))
        return (UNDEF);

    if (output != stdin)
        (void) fclose (output);

    PRINT_TRACE (2, print_string, "Trace: leaving print_obj_collstat_dict_noinfo");
    return (1);
}


int
close_print_obj_collstat_dict_noinfo (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_print_obj_collstat_dict_noinfo");
    

    PRINT_TRACE (2, print_string, "Trace: leaving close_print_obj_collstat_dict_noinfo");
    return (0);
}
