 ...
 *1 print.obj.tr_vecdict
 *2 print_obj_tr_vecdict (in_file, out_file, inst)
 *3   char *in_file;
 *3   char *out_file;
 *3   int inst;
 *4 init_print_obj_tr_vecdict (spec, unused)
 *5   "print.doc.textloc_file"
 *5   "print.doc.textloc_file.rmode"
 *5   "print.tr_file"
 *5   "print.tr_file.rmode"
 *5   "print.trace"
 *4 close_print_obj_tr_vecdict (inst)
...
extern int l_print_tr_vecdict (TR_VEC* tr_vec, int docno_fd, int textloc_index, SM_BUF* output, char* run_name, SM_INDEX_DOCDESC*, int*);
...
int
l_init_print_obj_tr_vecdict (spec, unused)
SPEC *spec;
char *unused;
{
    PRINT_TRACE (2, print_string, "Trace: entering l_init_print_obj_tr_vecdict");
	...
	*  parameters it needs. */
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving l_init_print_obj_tr_vecdict");
    return (0);
}

int
l_print_obj_tr_vecdict (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering l_print_obj_tr_vecdict");
    ...
    while (1 == (status = read_tr_vec (tr_vec_index, &tr_vec)) &&
        ...
        if ( l_print_tr_vecdict (&tr_vec, docno_fd, textloc_index, &output_buf, run_name, &doc_desc, contok_inst) == UNDEF )
			return UNDEF;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving l_print_obj_tr_vecdict");
    return (status);
}


int
l_close_print_obj_tr_vecdict (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering l_close_print_obj_tr_vecdict");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving l_close_print_obj_tr_vecdict");
    return (0);
}
