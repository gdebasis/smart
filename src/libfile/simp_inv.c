#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libfile/simp_inv.c,v 11.2 1993/02/03 16:50:23 smart Exp $";
#endif
...
static int overlay = offsetof(SIMP_INV, list) - sizeof(long);
static int overlay_align = ALIGN_EXTRA(offsetof(SIMP_INV, list) - sizeof(long));

int
create_simp_inv (file_name)
char *file_name;
{
    REL_HEADER rh;
...
long mode;
{
    ...
    if (mode & SCREATE) {
        if (UNDEF == create_simp_inv (file_name)) {
            return (UNDEF);
        }
        mode &= ~SCREATE;
    }
    index = open_direct(file_name, mode, S_RH_SINV);
    if (UNDEF != index && !(mode & SRDONLY)) {
	buf = &write_direct_buf[index];

	buf->buf = malloc(VAR_BUFF);
	if (NULL == buf->buf) {
	    set_error(errno, "couldn't alloc buf", "open_simp_inv");
	    buf->end = 0;
	    return UNDEF;
	}
	buf->end = VAR_BUFF;
...
SIMP_INV *simp_inv_entry;
{
    ...
    entry_num = simp_inv_entry->id_num;
...
SIMP_INV *simp_inv_entry;
{
    ...
    if (buf.size <= overlay) {
	set_error(SM_INCON_ERR, "buf too small", "read_simp_inv");
	return UNDEF;
    }

    simp_inv_entry->id_num = entry_num;
    (void) memcpy((char *)simp_inv_entry + sizeof(long), buf.buf, overlay);

    list_size = simp_inv_entry->num_list * sizeof(long);
    ...
    simp_inv_entry->list = (long *)(buf.buf + overlay + overlay_align);
...
SIMP_INV *simp_inv_entry;
{
    ...
    raw_list_size = simp_inv_entry->num_list * sizeof(long);
	...
	if (NULL == buf->buf) {
	    set_error(errno, "couldn't grow buf", "write_simp_inv");
	    buf->end = 0;
	    return UNDEF;
	}
    ...
    entry_num = simp_inv_entry->id_num;
    cp = buf->buf;
    (void) memcpy(cp, (char *)simp_inv_entry + sizeof(long), overlay);
    cp += overlay;
    if (overlay_align) {
	(void) memset(cp, '\0', overlay_align);
	cp += overlay_align;
    }
    (void) memcpy(cp, (char *)simp_inv_entry->list, raw_list_size);
    cp += raw_list_size;
    ...
    if (UNDEF == simp_inv_entry->id_num)
	simp_inv_entry->id_num = entry_num;
    return(status);
}
