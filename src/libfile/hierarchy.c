#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libfile/hierarchy.c,v 11.2 1993/02/03 16:50:14 smart Exp $";
#endif
...
static int overlay = offsetof(HIER, children) - sizeof(long);
static int overlay_align = ALIGN_EXTRA(offsetof(HIER, children) - sizeof(long));

int
create_hier(file_name)
char *file_name;
{
    REL_HEADER rh;
...
long mode;
{
    ...
    if (mode & SCREATE) {
        if (UNDEF == create_hier(file_name)) {
            return(UNDEF);
        }
        mode &= ~SCREATE;
    }
    index = open_direct(file_name, mode, S_RH_HIER);
    if (UNDEF != index && !(mode & SRDONLY)) {
	buf = &write_direct_buf[index];

	buf->buf = malloc(VAR_BUFF);
	if (NULL == buf->buf) {
	    set_error(errno, "couldn't alloc buf", "open_hier");
	    buf->end = 0;
	    return UNDEF;
	}
	buf->end = VAR_BUFF;
...
HIER *hier_entry;
{
    ...
    if (buf.size <= overlay) {
	set_error(SM_INCON_ERR, "buf too small", "read_hier");
	return UNDEF;
    }

    hier_entry->node_num = entry_num;
    (void) memcpy((char *)hier_entry + sizeof(long), buf.buf, overlay);

    list_size = hier_entry->num_children * sizeof(long);
    ...
    hier_entry->children = (long *)(buf.buf + overlay + overlay_align);
...
HIER *hier_entry;
{
    ...
    raw_list_size = hier_entry->num_children * sizeof(long);
	...
	if (NULL == buf->buf) {
	    set_error(errno, "couldn't grow buf", "write_hier");
	    buf->end = 0;
	    return UNDEF;
	}
    ...
    entry_num = hier_entry->node_num;
    cp = buf->buf;
    (void) memcpy(cp, (char *)hier_entry + sizeof(long), overlay);
    cp += overlay;
    if (overlay_align) {
	(void) memset(cp, '\0', overlay_align);
	cp += overlay_align;
    }
    (void) memcpy(cp, (char *)hier_entry->children, raw_list_size);
    cp += raw_list_size;
