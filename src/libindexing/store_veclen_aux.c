 ...
 *1 index.store.store_veclen_aux
 *2 store_veclen_aux (vec, unused, inst)
 *3   SM_VECTOR *vec;
 *3   char *unused;
 *3   int inst;
 *4 init_store_veclen_aux (spec, unused)
 *5   "index.store.trace"
 *5   "index.ctype.*.store_aux"
 *5   "index.doc_file"
 *5   "index.doc_file.rwmode"
 *4 close_store_veclen_aux (inst)
...
int
init_store_veclen_aux (spec, unused)
SPEC *spec;
char *unused;
{
    int new_inst;
    ...
    if (! check_context (CTXT_DOC)) {
        set_error (SM_ILLPA_ERR, "Illegal context", "store_veclen_aux");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_store_veclen_aux");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_store_veclen_aux");
 ...
 * pseudo-ctypes. */
int
store_veclen_aux (vec, unused, inst)
SM_VECTOR *vec;
char *unused;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering store_veclen_aux");
    PRINT_TRACE (6, print_vector, vec);

     if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "store_veclen_aux");
        return (UNDEF);
    }
    ip  = &info[inst];
   ...
   if (vec->num_conwt == 0)
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving store_veclen_aux");

    return (1);
}

int
close_store_veclen_aux (inst)
int inst;
{
    STATIC_INFO *ip;
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_store_veclen_aux");

     if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_store_veclen_aux");
        return (UNDEF);
    }
    ip  = &info[inst];
    ip->valid_info--;
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_store_veclen_aux");
