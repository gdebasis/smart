 ...
 *1 index.token_sect.token_sgml
 *2 token_sgml (pp_buf, t_sect, inst)
 *3   SM_BUF *pp_buf;
 *3   SM_TOKENSECT *t_sect;
 *3   int inst;
 *4 init_token_sgml (spec, unused)
 *5   "*.interior_char"
 *5   "*.interior_hyphenation"
 *5   "*.endline_hyphenation"
 *5   "*.fold_to_7bit"
 *5   "index.token_sect.trace"
 *4 close_token_sgml (inst)
...
int
init_token_sgml (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    if (UNDEF == lookup_spec_prefix (spec,
        ...
        return (UNDEF);
    PRINT_TRACE (2, print_string, "Trace: entering init_token_sgml");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_token_sgml");
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering token_sgml");
   ...
   /* Determine maximum size of buffer needed to hold all the document's
    ...
    if (size_needed > ip->size_token_buf) {
        ...
        if (NULL == (ip->token_buf = malloc ((unsigned) ip->size_token_buf))) {
            set_error (errno, "Out of space", "token_sgml");
            return (UNDEF);
        }
    }
    ...
    while (ptr < end_ptr) {
	    ...
	    if (ptr <= end_ptr && *ptr == ';') *buf_ptr++ = *ptr++;
            t_sect->tokens[t_sect->num_tokens].tokentype = SM_INDEX_IGNORE;
            break;
          case T_CN:
          default:
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving token_sgml");

    return (0);
}

int
close_token_sgml (inst)
int inst;
{
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_token_sgml");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_token_sgml");
        return (UNDEF);
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_token_sgml");
