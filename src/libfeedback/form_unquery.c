#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libfeedback/form_unnorm_query.c,v 11.0 1992/07/21 18:20:41 chrisb Exp $";
#endif
...
static SPEC_PARAM spec_args[] = {
    TRACE_PARAM ("feedback.form_unnorm_query.trace")
    };
...
int
init_form_unnorm_query (spec, unused)
SPEC *spec;
char *unused;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_form_unnorm_query");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_form_unnorm_query");
    return (new_inst);
}

int
form_unnorm_query (fdbk_info, new_query, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering form_unnorm_query");
    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "form_unnorm_query");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving form_unnorm_query");
    return (1);
}

int
close_form_unnorm_query (inst)
int inst;
{
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_form_unnorm_query");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_form_unnorm_query");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_form_unnorm_query");
    return (1);
}
