 ...
 *1 feedback.occ_info.inv_all_stats
 *2 occ_inv_all_stats (unused1, fdbk_info, inst)
 *3   char *unused1;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_occ_inv_all_stats (spec, unused)
 ...
 *4 close_occ_inv_all_stats (inst)
...
static int fdbk_fd;
int
init_occ_inv_all_stats (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_occ_inv_all_stats");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_occ_inv_all_stats");
    return (1);
}

int
occ_inv_all_stats (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering occ_inv_all_stats");
    ...
    for (i = 0; i < fdbk_info->num_occ; i++) {
        ...
        if (fdbk_info->occ[i].ctype >= num_ctypes) {
            set_error (SM_INCON_ERR, "Ctype too large", "occ_inv_all_stats");
            return (UNDEF);
        }
        if (1 != seek_inv (inv_fd[fdbk_info->occ[i].ctype], &inv) ||
            1 != read_inv (inv_fd[fdbk_info->occ[i].ctype], &inv)) {
	    /* Changed the following from return (0);. -mandar. */
	    printf("Concept %ld not in dictionary; ignored.\n", inv.id_num);
	    continue;
	}
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving occ_inv_all_stats");
...
int
close_occ_inv_all_stats (inst)
int inst;
{
    long i;

    PRINT_TRACE (2, print_string, "Trace: entering close_occ_inv_all_stats");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_occ_inv_all_stats");
    return (1);
}
