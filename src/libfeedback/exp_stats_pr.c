 ...
 *1 feedback.expand.exp_const_stats_prob
 *2 exp_const_stats_prob (unused1, fdbk_info, inst)
 *3   char *unused1;
 *3   FEEDBACK_INFO *fdbk_info;
 *3   int inst;
 *4 init_exp_const_stats_prob (spec, unused)
 *5    "feedback.fdbk_stat_file"
 *5   "feedback.fdbk_stat_file.rmode"
 *5   "feedback.expand.trace"
 *5   "feedback.ctype.*.num_expand"
 *4 close_exp_const_stats_prob (inst)
...
int
init_exp_const_stats_prob (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_exp_const_stats_prob");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_exp_const_stats_prob");
    return (1);
}

int
exp_const_stats_prob (unused, fdbk_info, inst)
char *unused;
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering exp_const_stats_prob");
    ...
    if (fdbk_info->num_rel > 0) {
        ...
        while (start_ctype_occ < end_occ) {
             ...
             *  2. prob(REL/i) * prob(i|rel)  ==  r/n  * r/R
             */
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving exp_const_stats_prob");
    return (1);
}

int
close_exp_const_stats_prob (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_exp_const_stats_prob");

    if (UNDEF == close_fdbkstats (fdbk_fd))
        return (UNDEF);

    PRINT_TRACE (2, print_string, "Trace: leaving close_exp_const_stats_prob");
    return (0);
}
...
OCC *ptr2;
{
    ...
    prob1 = (double) ptr1->rel_ret * (double) ptr1->rel_ret /
        (double) ptr1->rel_ret + (double) ptr1->nrel_ret;
    prob2 = (double) ptr2->rel_ret * (double) ptr2->rel_ret /
        (double) ptr2->rel_ret + (double) ptr2->nrel_ret;
    if (prob1 > prob2)
        return (-1);
    if (prob1 < prob2)
        return (1);
