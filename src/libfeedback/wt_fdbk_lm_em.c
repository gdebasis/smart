...
static SPEC_PARAM spec_args[] = {
    ...
    {"feedback.expanded_query", getspec_bool, (char *) &isAugmented}, // is the query expanded and contain the lambda values 
...
static float getLambda(VEC* vec, long keycon)
{
	CON_WT *conwtp = NULL, key_conwt;
	long   num_conwt;
	if (!isAugmented) {
	...
	key_conwt.con = keycon;
	key_conwt.wt = 0;

	// Search keycon in the augmented part of the vector
	num_conwt = half(vec->num_conwt);
	conwtp = bsearch(&key_conwt, vec->con_wtp + num_conwt, num_conwt, sizeof(CON_WT), compare_conwt);
	return conwtp == NULL? UNDEF : conwtp->wt; 
}
...
int inst;
{
    ...
    for (i = 0; i < fdbk_info->num_occ; i++) {
        ...
        for (k = 0; k < inv.num_listwt; k++) {
            ...
            if (tr_index < fdbk_info->tr->num_tr &&
                ...
                if ( fdbk_info->tr->tr[tr_index].rel ) {
					...
					lambda = getLambda(qvec, fdbk_info->occ[i].con) ;
		...
		if (fdbk_info->occ[i].rel_ret == 0)
	        ...
	        fdbk_info->occ[i].weight /= (float)fdbk_info->num_rel;
