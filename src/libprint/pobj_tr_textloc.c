#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libprint/pobj_tr_textloc.c,v 11.2 1993/02/03 16:54:11 smart Exp $";
#endif
 ...
 *0 print wiki object
 *1 print.obj.wiki
 *2 print_obj_tr_textloc (in_file, out_file, inst)
 *3   char *in_file;
 *3   char *out_file;
 *3   int inst;
 *4 init_print_obj_tr_textloc (spec, unused)
 *5   "print.tr_file"
 *5   "print.tr_file.rmode"
 *5   "print.trace"
 *4 close_print_obj_tr_textloc (inst)
...
static char* textloc_file, *q_textloc_file;
static int textloc_index, q_textloc_index;
...
static SPEC_PARAM spec_args[] = {
    ...
    {"print.tr_file.query.textloc_file",     getspec_dbfile, (char *) &q_textloc_file},
...
int
init_print_obj_tr_textloc (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_print_obj_tr_textloc");

    if (UNDEF == (textloc_index = open_textloc (textloc_file, textloc_mode)))
        return (UNDEF);
    if (UNDEF == (q_textloc_index = open_textloc (q_textloc_file, textloc_mode)))
        return (UNDEF);

    PRINT_TRACE (2, print_string, "Trace: leaving init_print_obj_tr_textloc");
    return (0);
}
...
int
print_obj_tr_textloc (in_file, out_file, inst)
char *in_file;
char *out_file;
int inst;
{
	...
	char* docname, *queryname;
	TEXTLOC textloc, q_textloc;

    PRINT_TRACE (2, print_string, "Trace: entering print_obj_tr_textloc");
    ...
    while (1 == (status = read_tr_vec (tr_vec_index, &tr_vec))) {
        ...
        q_textloc.id_num = tr_vec.qid;
        if (UNDEF == seek_textloc (q_textloc_index, &q_textloc) ||
            UNDEF == read_textloc (q_textloc_index, &q_textloc))
            return UNDEF;
        queryname = getFileName(q_textloc.file_name);
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving print_obj_tr_textloc");
    return (status);
}


int
close_print_obj_tr_textloc (inst)
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_print_obj_tr_textloc");

    if (UNDEF == close_textloc (textloc_index))
        return (UNDEF);
    if (UNDEF == close_textloc (q_textloc_index))
        return (UNDEF);

    PRINT_TRACE (2, print_string, "Trace: leaving close_print_obj_tr_textloc");
    return (0);
}
