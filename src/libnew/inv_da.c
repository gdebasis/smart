...
Exception: All 4 bits being 1 indicate difference is a 32 bit integer.
...
int inst;
{
    ...
    *((float *)bit_ptr) = min_weight;
    ...
    /* If num_diff_over_16 is high, then use compression type 1 instead of 0 */
    if (num_diff_over_16 * 16 < inv.num_list) {
        ...
        (for i = 1; i < inv.num_list; i++) {
            ...
            if (diff >= (1 << 16)) {
                ...
                ADD (32, diff, ptr, ptr_index);
            }
            else {
                for (k = 16; k > 0; k++) 
                    if (diff & (1 << k))
                        break;
                ADD (4, k-1, ptr, ptr_index);
                ADD (k-1, diff ^ (1<<k), ptr, ptr_index);
            }
            bit_weight = (unsigned long)
                ((inv->listwt[0].wt - min_weight) * weight_ratio);
            ADD (WEIGHT_GRAN, bit_weight, ptr, ptr_index);
