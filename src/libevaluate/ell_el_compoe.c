#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libevaluate/ell_el_comp_over_equal.c,v 11.0 1992/07/21 18:20:34 chrisb Exp chrisb $";
#endif
 ...
 *0 Evaluate a set of runs, counting queries better or same as first run
 *2 ell_el_comp_over_equal (eval_list_list, eval_list, inst)
 *3   EVAL_LIST_LIST *eval_list_list;
 *3   EVAL_LIST *eval_list;
 *3   int inst;
 *4 init_ell_el_comp_over_equal (spec, unused)
 *5   "eval.trace"
 *4 close_ell_el_comp_over_equal (inst)
 ...
 *7 of queries which had values better or equal to the corresponding query of
 *7 the first run.
***********************************************************************/
...
static char *convert_desc = "   Number of queries with evaluation values better or equal to the first run";

int
init_ell_el_comp_over_equal (spec, unused)
SPEC *spec;
char *unused;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ell_el_comp_over_equal");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ell_el_comp_over_equal");

    return (new_inst);
}

int
ell_el_comp_over_equal (eval_list_list, eval_list, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering ell_el_comp_over_equal");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ell_el_comp_over_equal");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ell_el_comp_over_equal");
    return (1);
}


int
close_ell_el_comp_over_equal (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_ell_el_comp_over_equal");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_ell_el_comp_over_equal");
    return (0);
}
