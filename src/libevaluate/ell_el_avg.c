#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libevaluate/ell_el_avg.c,v 11.0 1992/07/21 18:20:34 chrisb Exp chrisb $";
#endif
 ...
 *0 Convert per_run,per_query evaluation to per_run averaged over queries
 *2 ell_el_avg (eval_list_list, eval_list, inst)
 *3   EVAL_LIST_LIST *eval_list_list;
 *3   EVAL_LIST *eval_list;
 *3   int inst;
 *4 init_ell_el_avg (spec, unused)
 *5   "eval.trace"
 *4 close_ell_el_avg (inst)
 ...
 *7 gives the averages over all that run's queries of each evaluation
 *7 figure.
***********************************************************************/
...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ell_el_avg");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ell_el_avg");

    return (new_inst);
}

int
ell_el_avg (eval_list_list, eval_list, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering ell_el_avg");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ell_el_avg");
        return (UNDEF);
    }
    ip  = &info[inst];

    /* Reserve space and initialize to 0 the average results */
    if (UNDEF == init_results (ip, eval_list_list, eval_list))
        return (UNDEF);
    /* Initialize the description field */
    if (3 + strlen (eval_list_list->description) +
        strlen ("         Average over all queries") > ip->num_desc_buf) {
        if (0 != ip->num_desc_buf)
            (void) free (ip->desc_buf);
        ip->num_desc_buf = 2 * (3 + strlen (eval_list_list->description) +
                                strlen ("         Average over all queries"));
    ...
    for (num_run = 0; num_run < eval_list_list->num_eval_list; num_run++) {
        ...
        if (q_eval_list->description != NULL) {
            if (7 + strlen (ip->desc_buf) + strlen (q_eval_list->description) >
                ip->num_desc_buf) {
                ip->num_desc_buf += 7 + strlen (ip->desc_buf) +
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ell_el_avg");
    return (1);
}


int
close_ell_el_avg (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_ell_el_avg");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_ell_el_avg");
    return (0);
}
