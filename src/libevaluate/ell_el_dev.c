#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libevaluate/ell_el_dev.c,v 11.0 1992/07/21 18:20:34 chrisb Exp chrisb $";
#endif
 ...
 *0 Convert per_run,per_query evaluation to per_run giving deviations
 *2 ell_el_dev (eval_list_list, eval_list, inst)
 *3   EVAL_LIST_LIST *eval_list_list;
 *3   EVAL_LIST *eval_list;
 *3   int inst;
 *4 init_ell_el_dev (spec, unused)
 *5   "eval.trace"
 *4 close_ell_el_dev (inst)
 *7 Take an already evaluated set of runs, with evaluations for each query
 *7 in each run, and produce an EVAL_LIST object which for each run
 *7 gives the standard deviation over all that run's queries of each evaluation
 *7 figure.
***********************************************************************/
...
{\
    double sum = 0.0;\
    for (num_query = 0; \
         num_query < q_eval_list->num_eval;\
         num_query++) { \
        sum += (avg_eval->x - q_eval_list->eval[num_query].x) * \
            (avg_eval->x - q_eval_list->eval[num_query].x);\
    }\
    avg_eval->x = (float) sqrt (sum);\
}

static char *convert_desc = "   Deviations from the average over all queries";

int
init_ell_el_dev (spec, unused)
SPEC *spec;
char *unused;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_ell_el_dev");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_ell_el_dev");

    return (new_inst);
}

int
ell_el_dev (eval_list_list, eval_list, inst)
...
int inst;
{
    ...
    EVAL *avg_eval, *query_eval;

    PRINT_TRACE (2, print_string, "Trace: entering ell_el_dev");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "ell_el_dev");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    for (num_run = 0; num_run < eval_list_list->num_eval_list; num_run++) {
        ...
        avg_eval = &eval_list->eval[num_run];
        ...
        /* Now go through again and find the standard deviations */
        STD_DEV (exact_recall);
        STD_DEV (exact_precis);
        STD_DEV (exact_rel_precis);
        STD_DEV (av_recall_precis);
        STD_DEV (int_av_recall_precis);
        STD_DEV (int_nrp_av_recall_precis);
        STD_DEV (av_fall_recall);
        STD_DEV (R_recall_precis);
        STD_DEV (av_R_precis);
        STD_DEV (int_R_recall_precis);
        STD_DEV (int_av_R_precis);
        for (i = 0; i < avg_eval->num_cutoff; i++) {
            STD_DEV (recall_cut[i]);
            STD_DEV (precis_cut[i]);
            STD_DEV (rel_precis_cut[i]);
        }
        for (i = 0; i < avg_eval->num_rp_pts; i++)
            STD_DEV (int_recall_precis[i]);
        for (i = 0; i < avg_eval->num_fr_pts; i++)
            STD_DEV (fall_recall[i]);
        for (i = 0; i < avg_eval->num_prec_pts; i++) {
            STD_DEV (R_prec_cut[i]);
            STD_DEV (int_R_prec_cut[i]);
        }
    }
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving ell_el_dev");
    return (1);
}


int
close_ell_el_dev (inst)
int inst;
{
    STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_ell_el_dev");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_ell_el_dev");
    return (0);
}
