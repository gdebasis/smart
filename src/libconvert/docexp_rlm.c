...
static SPEC_PARAM spec_args[] = {
    ...
    {"docexp_rlm.nnn_doc_file",         getspec_dbfile,   (char *) &nnn_dvec_file},
    {"docexp_rlm.doc_file",             getspec_dbfile,   (char *) &dvec_file},
    TRACE_PARAM ("docexp_rlm.trace")
};
...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering/leaving init_docexp_rlm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_docexp_rlm");
    return (new_inst);
}
...
int docexp_rlm (VEC* invec, VEC* ovec, TR_VEC* tr_vec, int inst)
{
	TR_TUP  *tr_tup;
	long    i, j;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering docexp_rlm");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "docexp_rlm");
        return (UNDEF);
    }
	ip = &info[inst];
	...
	for (i = 0; i < tr_vec->num_tr; i++) {
		...
		for (conwtp = dvec.con_wtp; conwtp < &dvec.con_wtp[dvec.ctype_len[0]]; conwtp++) {
			...
			if (ip->conwt_buff[conwtp->con].wt == 0 && sm_trace==1) {
			...
			ip->conwt_buff[conwtp->con].wt += (1-alpha-beta)*tr_tup->sim*conwtp->wt/(float)doclen; 
		}
	}

	if (invec->id_num.id == ip->max_did && sm_trace==1) {
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving docexp_rlm");
    return (1);
}

int close_docexp_rlm (int inst)
{
	STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_docexp_rlm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_docexp_rlm");
    return (0);
}
