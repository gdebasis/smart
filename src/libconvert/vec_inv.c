#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/liblocal/libconvert/lvec_inv_tmp.c,v 11.2 1993/02/03 16:51:49 smart Exp $";
#endif
 ...
 *8     eg 670, 1 Gbyte should work.  (All of this is using vec_inv_tmp;
 *8     if using vec_inv then virt_mem_usage should be a bit less than
 ...
 *9 Warning: Algorithm currently assumes sizeof (LISTWT) divides
...
STATIC_INFO *ip;
{
    ...
    for (con = 0; con < ip->num_list_hd; con++) {
        ...
        if (ip->list_hd[con].num_linked > 0) {
            ...
            if (UNDEF == seek_inv (ip->inv_fd, &inv) ||
                UNDEF == write_inv (ip->inv_fd, &inv))
                return (UNDEF);
        }
    }
...
long con;                   /* Current concept about to treat */
{
    ...
    for (aux_buf = 0; aux_buf < ip->num_aux_hd; aux_buf++) {
        ...
        if (aux_hd_ptr->num_list > aux_hd_ptr->current_list)
            total_list_length += (aux_hd_ptr->num_list -
                                  aux_hd_ptr->current_list) * sizeof (LISTWT);
    }
    ...
    for (aux_buf = 0; aux_buf < ip->num_aux_hd; aux_buf++) {
        ...
        list_start = aux_hd_ptr->current_list * sizeof (LISTWT);
        ...
        *end_temp_file = '\0';
        ...
        if (list_length_wanted > 0) {
            ...
            list_page_length_wanted = list_length_wanted + list_start -
                list_page_start;
            if ((LISTWT *) -1 == (aux_hd_ptr->listwt_buf = (LISTWT *)
                                 mmap ((caddr_t) 0,
                ...
                return (UNDEF);
            aux_hd_ptr->min_incore_list = list_page_start / sizeof (LISTWT);
        ...
        if (NULL == (aux_hd_ptr->num_list_buf = (long *)
            ...
            NULL == (aux_hd_ptr->listwt_buf = (LISTWT *)
        ...
        aux_hd_ptr->end_incore_list = aux_hd_ptr->current_list +
                                      (list_length_wanted / sizeof (LISTWT));
