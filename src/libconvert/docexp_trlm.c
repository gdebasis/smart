...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering/leaving init_docexp_trlm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_docexp_trlm");
    return (new_inst);
}
...
int docexp_trlm (VEC* invec, VEC* ovec, TR_VEC* tr_vec, int inst)
{
	TR_TUP      *tr_tup;
	long        i, j, rel_count = 0;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering docexp_trlm");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "docexp_trlm");
        return (UNDEF);
    }
	ip = &info[inst];
	...
	// Turn on the rel field of each document in the tr_vec, because
	// the LDA estimator uses the rel field. Don't feed in the 'self' doc in the LDA estimate
	for (tr_tup = tr_vec->tr; tr_tup < &tr_vec->tr[tr_vec->num_tr]; tr_tup++) {
		tr_tup->rel = (tr_tup->did == invec->id_num.id)? 0 : 1;
	}
	...
	if (UNDEF == init_lda_est_inst(&ip->ldamodel, nnn_doc_fd, tr_vec, invec, multifaceted)) {
		fprintf(stderr, "TRLM expansion failed for document %d\n", invec->id_num.id);
		return 1;
	}
	lda_est(&ip->ldamodel);
	...
	for (i = 0; i < tr_vec->num_tr; i++) {
		...
		p_D_Q = !multifaceted? tr_tup->sim : getMarginalizedQueryGenEstimate(&ip->ldamodel, rel_count, NULL, 0);

		// Iterate for every word in this document
		for (conwtp = dvec.con_wtp; conwtp < &dvec.con_wtp[dvec.ctype_len[0]]; conwtp++) {
			ip->conwt_buff[conwtp->con].con = conwtp->con; 
			p_w_D = compute_marginalized_P_w_D(&ip->ldamodel, conwtp, i);
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving docexp_trlm");
    return (1);
}


int close_docexp_trlm (int inst)
{
	STATIC_INFO *ip;
    PRINT_TRACE (2, print_string, "Trace: entering close_docexp_trlm");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_docexp_trlm");
    return (0);
}
