 ...
 *0 Reweight vector using two part weighting scheme with no normalization
 *1 convert.wt_ctype.weight_tfidf
 *2 weight_tfidf (vec, unused, inst)
 *3   VEC *vec;
 *3   char *unused;
 *3   int inst;
 *4 init_weight_tfidf (spec, param_prefix)
 *5   "convert.weight.trace"
 *5   "<prefix>*.tri_weight"
 *4 close_weight_tfidf (inst)

 *7 Weight vector according the value of the parameter given by 
 *7 the concatenation of param_prefix and "tri_weight" or
 *7 "query weight".  
 *7 Normally, param_prefix will have some value like "doc.ctype.1." in order
 *7 to obtain the ctype dependant weight.  Ie. "doc.ctype.1.tri_weight"
 ...
 *7     Third char gives the normalization procedure but is ignored here.
 ...
 *7          THIS IS NOT DONE HERE, BUT MAY BE DONE AT A HIGHER LEVEL!
 *7 
 ...
 *7 
 *7       3. ALL OF THE FOLLOWING ARE IGNORED HERE
 *7           "none"     : norm_weight = new_wt
...
int
init_weight_tfidf (spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_weight_tfidf");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_weight_tfidf");

    return (new_inst);
}

int
weight_tfidf (vec, unused, inst)
VEC *vec;
char *unused;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering weight_tfidf");
    PRINT_TRACE (6, print_vector, vec);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "weight_tfidf");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving weight_tfidf");
    return (1);
}


int
close_weight_tfidf (inst)
int inst;
{
    STATIC_INFO *ip;

    PRINT_TRACE (2, print_string, "Trace: entering close_weight_tfidf");

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "close_weight_tfidf");
        return (UNDEF);
    }
    ip  = &info[inst];
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_weight_tfidf");
